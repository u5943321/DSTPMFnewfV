...
Theorem treplace_tsstt:
(∀t i new. treplace i new t = tsstt (Bound i) new t) ∧
(∀s i new. sreplace i new s = ssstt (Bound i) new s)
Proof
ho_match_mp_tac better_tm_induction >> rw[treplace_def,tsstt_def,MAP_EQ_f]
QED
...
Theorem treplace_tsubst:
(∀tm n s i new.
  (n,s) ∈ tfv tm ∧ Bound i ∉ tsubtm tm ⇒
  treplace i new (tsubst (n,s) (Bound i) tm) =
  tsstt (Var n s) new tm) ∧
(∀st n s i new.
  (n,s) ∈ sfv st ∧ Bound i ∉ ssubtm st ⇒
  sreplace i new (ssubst (n,s) (Bound i) st) =
  ssstt (Var n s) new st)
Proof
 rw[treplace_tsstt,tsubst_tsstt] (* 2 *)
 >- (irule $ cj 1 tsstt_tsstt  >> rw[Var_tsubtm_tfv]) >>
 irule $ cj 2 tsstt_tsstt >> rw[Var_tsubtm_tfv]
QED
...
Theorem treplace_id:
(∀t i new. i ∉ tbounds t ⇒ treplace i new t = t) ∧
(∀st i new. i ∉ sbounds st ⇒ sreplace i new st = st) 
Proof
ho_match_mp_tac better_tm_induction >> rw[] (* 4 *)
>- (gs[treplace_def,tbounds_def])
>- (gs[treplace_def,tbounds_def,MEM_MAP] >> rw[MAP_fix] >>
    metis_tac[])
>- gs[treplace_def,tbounds_def] >>
gs[treplace_def,tbounds_def,MEM_MAP] >> rw[MAP_fix] >>
metis_tac[]
...
∀sl t i n. n < LENGTH sl ⇒
 EL n (specsl i t sl) = sreplace (i + n) t (EL n sl)
Proof
...
Theorem trpl_treplace:
(∀t i new. 
   (∀n0 s0. (n0,s0) ∈ tfv t ⇒ sbounds s0 = ∅) ⇒
   treplace i new t = trpl i new t) ∧
(∀s i new.
   (∀n0 s0. (n0,s0) ∈ sfv s ⇒ sbounds s0 = ∅) ⇒
   sreplace i new s = srpl i new s)
Proof
ho_match_mp_tac better_tm_induction >>
simp[MEM_MAP,tfv_thm,treplace_def,trpl_def,PULL_EXISTS] >>
rw[] (* 4 *)
>- (irule $ cj 2 treplace_id >>
   ‘sbounds s = {}’ by metis_tac[] >>
   gs[])
>- metis_tac[]
>> (rw[MAP_EQ_f] >> metis_tac[])
QED



Theorem tfv_treplace_SUBSET:
 (∀t i new. 
            (∀n0 s0. (n0,s0) ∈ tfv t ⇒ sbounds s0 = ∅) ⇒
            tfv t ⊆ tfv (treplace i new t)) ∧
 (∀s i new. 
            (∀n0 s0. (n0,s0) ∈ sfv s ⇒ sbounds s0 = ∅) ⇒
           sfv s ⊆ sfv (sreplace i new s))
Proof
 rw[] (* 2 *)
 >- (Cases_on ‘i ∈ tbounds t’ 
    >- (drule_then assume_tac $ cj 1 trpl_treplace >>
    simp[] >>
    drule_all_then assume_tac $ cj 1 tfv_trpl >>
    simp[]) >>
    ‘(treplace i new t)= t’  by metis_tac[treplace_id] >>
    simp[]) >>
 Cases_on ‘i ∈ sbounds s’ (* 2 *)
 >- (drule_then assume_tac $ cj 2 trpl_treplace >>
     simp[] >>
     drule_all_then assume_tac $ cj 2 tfv_trpl >>
     simp[]) >>
 ‘(sreplace i new s)= s’  by metis_tac[treplace_id] >>
 simp[]     
QED
...
Proof
 ...
 qexists ‘sreplace n t s’ >>
 pop_assum (assume_tac o GSYM) >> gs[] >>
 ‘sfv s ⊆ sfv (sreplace n t s)’
  by (irule $ cj 2 tfv_treplace_SUBSET >>
     metis_tac[]) >>
 rw[] (* 2 *)
 >- gs[SUBSET_DEF]>>
...
>- (drule_then assume_tac wfcod_no_bound >>
   ...
   specsl 0 (tinst σ h) (MAP (sinst σ) t) ’
    by (irule MAP_sinst_specsl >> rw[] (* 2 *)
       >- metis_tac[]
       >- gs[no_bound_def] >>
       irule SUBSET_TRANS >>
       first_x_assum $ irule_at Any >>
       ...
       qexists ‘n’ >> simp[] >>
       ‘sfv (EL n t) ⊆ sfv (sreplace n h (EL n t))’
        suffices_by metis_tac[SUBSET_DEF] >>
       irule $ cj 2 tfv_treplace_SUBSET >> metis_tac[]) >>
    gs[])
...
‘sfv y ⊆ FDOM σ’
  by (irule SUBSET_TRANS >>
     ...
     irule SUBSET_TRANS >>
       ...
       drule_then assume_tac specsl_EL >> simp[] >>
       qexists ‘n’ >> simp[] >>
       ‘sfv (EL n sl) ⊆ sfv (sreplace n t (EL n sl))’
        suffices_by metis_tac[SUBSET_DEF] >>
       irule $ cj 2 tfv_treplace_SUBSET >> metis_tac[]) >>
(*maybe lemma*)       
...
Theorem slreplace_slabs:
∀l i n st new.
(∀m. m < LENGTH l ⇒ i + m ∉ sbounds (EL m l) )
(*
(∀y. MEM y l ⇒ i ∉ sbounds y) *) ⇒
slreplace i new (slabs (n,st) i l) =
MAP (ssubst (n,st) new) l
Proof
Induct_on ‘l’ >> simp[slabs_def,slreplace_def] >>
rw[] (* 2 *)
>- (rw[treplace_tsstt,tsubst_tsstt] >>
   irule $ cj 2 tsstt_tsstt1 >>
   gs[Bound_tsubtm] >>
   first_x_assum $ qspecl_then [‘0’] assume_tac >> gs[]) >>
...
Theorem MAP_sinst_slabs:
 ...
 slabs (nn,sinst σ ast) i
          (MAP (sinst (σ |+ ((an,ast),Var nn (sinst σ ast)))) l) =
        MAP (sinst σ) (slabs (an,ast) i l)
Proof
Induct_on ‘l’ >> simp[slabs_def] >> rw[] (* 2 *)
>- (irule ssubst_sinst >>
...
‘IMAGE PRE (slbounds sl DELETE (i + 1) DELETE 0) =
 IMAGE PRE (slbounds sl DELETE 0) DELETE i’
  ...
  (rw[Once EXTENSION,EQ_IMP_THM] (* 3 *)
   ...
   qexists ‘x'’ >> simp[]) >>
simp[] >>
drule_then assume_tac $ cj 2 treplace_eliminate >>
simp[] >> rw[EXTENSION] >> metis_tac[]
QED
...
(*Theorem tprpl_mk_bmap_tabs:
(∀tm i.
   (n,s) ∉ tfv tm  ⇒
   tprpl
   (mk_bmap (MAP (tabs (n,s) i) l0)) tm =
   tabs (n,s) i (tprpl (mk_bmap l0) tm)) ∧
(∀st i.
   (n,s) ∉ sfv st  ⇒ sprpl (mk_bmap (MAP (tabs (n,s) i) l0)) st =
   sabs (n,s) i (sprpl (mk_bmap l0) st))
Proof
ho_match_mp_tac better_tm_induction >>
...
>- (‘¬(n = s0 ∧ s = st)’ by metis_tac[] >> simp[])
>- metis_tac[] 
>- gs[FAPPLY_mk_bmap,FDOM_mk_bmap,EL_MAP]
>- gs[FAPPLY_mk_bmap,FDOM_mk_bmap,EL_MAP]
>- gs[FAPPLY_mk_bmap,FDOM_mk_bmap,EL_MAP]
>- gs[FAPPLY_mk_bmap,FDOM_mk_bmap,EL_MAP,tabs_def] >>
metis_tac[]
QED
*)
...
Theorem ok_abs_slabs:
ok_abs (slabs (n,s) i l) ⇒ ok_abs l
Proof
rw[] >> metis_tac[slabs_id,NOT_ok_abs]
QED
