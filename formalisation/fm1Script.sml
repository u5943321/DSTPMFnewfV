...
Definition sname_def:
sname (St n _) = n
End


Definition tsname_def:
tsname = sname o sort_of
End
...
is_EQ f ⇔ ∃t1 t2. f = EQ t1 t2
End

Definition dest_eq_def:
dest_eq (Pred "=" [t1;t2]) = (t1,t2)
End
...
Definition vsname_def:
  vsname (n,s) = sname s
End
  
Definition presname_def:
  presname σ ⇔ ∀v. v ∈ FDOM σ ⇒ ¬(is_bound (σ ' v)) ∧
                                 tsname (σ ' v) = vsname v
End                                     


Theorem tsname_tinst:
  (∀t. ¬(is_bound t) ∧ presname σ ⇒
  tsname (tinst σ t) = tsname t) ∧
  (∀s. presname σ ⇒
   sname (sinst σ s) = sname s)
Proof
 ho_match_mp_tac better_tm_induction >>
 gs[is_bound_alt,tsname_def,sort_of_def,sname_def,tinst_def] >> rw[] >>
 Cases_on ‘(s0,s) ∈ FDOM σ’ >> simp[sort_of_def] >>
 gs[presname_def] >>
 first_x_assum $ drule_then assume_tac >> gs[tsname_def,vsname_def]
QED  


Theorem wft_not_bound:
  wft Σ t ⇒ ¬(is_bound t)
Proof
  rw[is_bound_alt] >>CCONTR_TAC >>
  drule_then assume_tac $ cj 1 wft_no_bound >>
  gs[tbounds_thm]
QED  
...
Proof
rw[EQ_def,Once EXTENSION] >> metis_tac[]
QED            
                      

Theorem presname_FUPDATE:
  presname σ ⇒ ∀v t. ¬(is_bound t) ∧ vsname v = tsname t ⇒ presname (σ |+ (v,t))
Proof  
 rw[presname_def,FAPPLY_FUPDATE_THM] (*2 *)
 >> (Cases_on ‘v' = v’ >> gs[])
QED 
 

Theorem presname_DRESTRICT:
presname σ ⇒ presname (DRESTRICT σ s)
Proof
gs[DRESTRICT_DEF,presname_def]
QED
...
>- (CCONTR_TAC >>
   ...
   gs[SUBSET_DEF] >> metis_tac[wft_no_bound,MEMBER_NOT_EMPTY]) >>
(CCONTR_TAC >>
   gs[GSYM MEMBER_NOT_EMPTY] >>
...
Theorem presname_cvmap:
∀vs. FINITE vs ⇒ presname (cvmap vs)
Proof        
rw[presname_def,FDOM_cvmap] (* 2 *)
>- (drule_then assume_tac FAPPLY_cvmap >>
   Cases_on ‘v’ >> first_x_assum $ drule_then assume_tac >>
   gs[is_bound_alt]) >>
drule_then assume_tac FAPPLY_cvmap >>
Cases_on ‘v’ >> first_x_assum $ drule_then assume_tac >>
gs[tsname_def,vsname_def,sort_of_def]
QED   
...
Theorem frename_eq:
(frename (n,s) nn (EQ t1 t2)) = EQ (trename (n,s) nn t1) (trename (n,s) nn t2)
Proof
rw[frename_alt,EQ_def]
QED   

(*      
Theorem sort_of_trename:
   ∀t.
    ¬(is_bound t) ⇒ sort_of (trename (n,s) nn t) = srename (n,s) nn (sort_of t)
Proof
  Induct_on ‘t’ >> gs[is_bound_alt,sort_of_def,trename_alt] >> rw[] (* 2 *)
  >- gs[sort_of_def] >> 
  rw[trename_tinst_tfv] >>
  ‘sinst (rnmap (n,s) nn (sfv (sort_of t))) (sort_of t) =
   sinst (rnmap (n,s) nn (tfv t)) (sort_of t)’ by cheat >> gs[] >>
  irule cstt_sort_of_tinst' >> simp[] >>
...
Theorem tsname_trename:
∀t. ¬is_bound t ⇒ (tsname (trename (n,s) nn t)) = tsname t
Proof
Cases_on ‘t’ >> gs[tsname_def,trename_alt,sort_of_def,is_bound_alt] (* 2 *)
>- (Cases_on ‘s0 = n ∧ s' = s’ >> simp[sort_of_def] >>
   Cases_on ‘s'’ >> gs[trename_alt,sname_def]) >>
Cases_on ‘s'’ >> gs[trename_alt,sname_def]
QED   


Theorem presname_rnmap:
∀vs. FINITE vs ⇒ presname (rnmap (n,s) nn vs)
Proof
rw[presname_def,FDOM_rnmap] (* 2 *)
>- (Cases_on ‘v’ >> drule_all_then assume_tac FAPPLY_rnmap >>
   gs[is_bound_alt,trename_alt] >>
   Cases_on ‘q = n ∧ r = s’ >> simp[]) >>
Cases_on ‘v’ >> drule_all_then assume_tac FAPPLY_rnmap >> gs[] >>
Cases_on ‘r’ >> gs[vsname_def,trename_alt,tsname_def,sort_of_def] >>
Cases_on ‘q = n ∧ St s' l = s’ >> simp[]
>- rw[sort_of_def] >>
rw[sort_of_def,sname_def]
QED
