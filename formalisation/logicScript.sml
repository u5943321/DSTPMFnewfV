...
(*only work for wf sig, otherwise the sort of function terms problematic*)        
Definition tfv_def:
  tfv (Var n s) = {(n,s)} ∪ sfv s ∧
  tfv (Fn n s tl) = sfv s ∪ BIGUNION (set (MAP tfv tl)) ∧
...
Theorem tfv_thm[simp]:
  tfv (Var n s) = {(n,s)} ∪ sfv s ∧
  tfv (Fn n s tl) = BIGUNION {tfv t | MEM t tl} ∧
...
Definition wffVar_def:
  wffVar Σf (fVar P vl tl) ⇔
    ...
    EVERY (wft Σf) tl ∧
    IS_SOME (tlmatch {} (MAP (UNCURRY Var) vl) tl FEMPTY)
End
...
Definition wff_def[simp]:
  ...
  (wff (Σf,Σp) (Pred p tl) ⇔
     ispsym Σp p ∧
     IS_SOME (tlmatch {} (MAP (UNCURRY Var) (Σp ' p)) tl FEMPTY)) ∧
...
Proof
  ...
  rw[MAP_EQ_f] (* 2 *)
  >- Cases_on ‘s’ >>
     gs[tinst_def,MAP_EQ_f,stms_def] >>  rw[] >>
     last_x_assum irule >>
     irule_at Any DRESTRICT_SUBSET 


>> first_x_assum irule >> irule_at Any DRESTRICT_SUBSET >>
...
Definition wffsig_def:
  wffsig Σf ⇔
  ∀f. f ∈ FDOM Σf ⇒
      sfv (foutput Σf f) ⊆ BIGUNION {tfv (Var n s) | MEM (n,s) (finput Σf f)}
End      
*)
...
Theorem wft_Fn:
   tl1 = tl2 ∧
   wft Σ (Fn f s1 tl1) ∧ wft Σ (Fn f s2 tl2) ⇒
   (Fn f s1 tl1) = (Fn f s2 tl2)
Proof
  rw[wft_def]
QED  
...
Proof
  ...
  >- (‘(s0,s) ∈ FDOM σ1 <=> (s0,s) ∈ FDOM σ2’
     ...
     >- (gs[DRESTRICT_DEF,fmap_EXT,EXTENSION] >>
        metis_tac[]) >>
     first_x_assum irule >> gs[wft_def] >>
     irule DRESTRICT_SUBSET >>
     first_x_assum $ irule_at Any >>
     rw[SUBSET_DEF])
  >- (gs[wft_def] >> first_x_assum irule >>
     irule DRESTRICT_SUBSET >>
     first_x_assum $ irule_at Any >> gs[])
  >- (gs[MAP_EQ_f,wft_def]>> rw[] >>
     first_x_assum irule >>
     rw[] >> irule DRESTRICT_SUBSET >>
     first_x_assum $ irule_at Any >>
     rw[SUBSET_DEF] >> metis_tac[]) >>
  gs[MAP_EQ_f,wft_def,EVERY_MEM] >> rw[] >>
  first_x_assum irule >>
  rw[] >> irule DRESTRICT_SUBSET >> 
  first_x_assum $ irule_at Any >> 
  rw[SUBSET_DEF] >> metis_tac[]
