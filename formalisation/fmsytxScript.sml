...
rw[fabs_def,MAP_fix] (* 2 *)
>- (irule LIST_EQ >> simp[LENGTH_abssl] >> rw[] >>
...
∀fm i n s.
 (∀n0 s0. (n0,s0) ∈ ffv fm ⇒ (n,s) ∉ sfv s0) ⇒
 fabs (n,s) i fm = abs (n,s) i fm
Proof
Induct_on ‘fm’ >>
simp[fabs_def,abs_def,ffv_thm,tfv_thm,MAP_EQ_f,PULL_EXISTS]>>
rw[] (* 7 *)
...
Proof
 ...
 rw[finst_def,fabs_def,
    MAP_MAP_o,MAP_EQ_f] (* 2 *)
 >- (irule MAP_sinst_abssl >> gs[SUBSET_DEF] >> metis_tac[]) >>
 irule tabs_tinst >> simp[] >>
 gs[SUBSET_DEF] >> metis_tac[]
QED         
...
>- (disj1_tac >> simp[IN_slbounds] >>
   gs[MEM_EL] >> qexists ‘n'’ >> gs[LENGTH_abssl] >>
   drule_then assume_tac abssl_EL >>
   simp[] >> metis_tac[tabs_tbounds_in])
>- metis_tac[tabs_tbounds_in]
QED        
...
rw[] (* 2 *)
>- (‘slbounds l ⊆
    slbounds (abssl (n,s') i l)’
  suffices_by gs[SUBSET_DEF,UNION_DEF] >>
  rw[SUBSET_DEF,IN_slbounds,LENGTH_abssl] >>
  qexists ‘m’ >> drule_then assume_tac abssl_EL >>
  simp[] >>
  ‘sbounds (EL m l) ⊆
   sbounds (sabs (n,s') (i + m) (EL m l))’
   suffices_by gs[SUBSET_DEF] >>
  irule $ cj 2 tbounds_tabs_SUBSET >> metis_tac[MEM_EL]) >>
...
Proof
Induct_on ‘f’ >> simp[fabs_def,fbounds_thm] (* 4 *)
>- (simp[MEM_MAP,PULL_EXISTS] >>
   rw[] >>
   rw[Once EXTENSION,EQ_IMP_THM,PULL_EXISTS] (* 3 *)
   >- (Cases_on ‘(n,s') ∈ tfv y’ (* 2 *)
...
simp[PULL_EXISTS,MEM_MAP] >> rw[] (* 2 *)
>- (drule_then assume_tac $ iffLR MEM_EL >> gs[] >>
   drule_then assume_tac slbounds_abssl >>
   first_x_assum $ drule_then assume_tac >>
   ...
   Cases_on ‘∃y. MEM y l0 ∧ (n,s') ∈ tfv y’ (* 2 *)
   >- (gs[] >>
      ‘BIGUNION {tbounds t | (∃y. t = tabs (n,s') i y ∧ MEM y l0)} = {i} ∪ BIGUNION {tbounds t | MEM t l0}’
       by (irule BIGUNION_tbounds' >>
           gs[] >> metis_tac[]) >>
      gs[EXTENSION] >> metis_tac[]) >>
   ‘{tbounds t | (∃y. t = tabs (n,s') i y ∧ MEM y l0)} = {tbounds t | MEM t l0}’
    by (rw[Once EXTENSION] >> metis_tac[tabs_id]) >>
   gs[EXTENSION] >> metis_tac[]) >>
‘BIGUNION {tbounds t | (∃y. t = tabs (n,s') i y ∧ MEM y l0)} = {i} ∪ BIGUNION {tbounds t | MEM t l0}’
  by (irule BIGUNION_tbounds' >> gs[] >> metis_tac[]) >>
Cases_on ‘∃s. MEM s l ∧ (n,s') ∈ sfv s’ (* 2 *)
>- (gs[] >> drule_then assume_tac $ iffLR MEM_EL >>
   gs[] >>
   ‘slbounds (abssl (n,s') i l) = {i} ∪ slbounds l’
    by (irule slbounds_abssl >> gs[] >>
       metis_tac[]) >> simp[] >>
   gs[EXTENSION] >> metis_tac[]) >>
‘(abssl (n,s') i l) = l’
 by metis_tac[NOTIN_slbounds_abssl] >>
gs[EXTENSION] >> metis_tac[]
QED
...
>- metis_tac[wft_wfs] >>
Cases_on ‘(n,s) ∈ ffv f’ (* 2 *)
>- (drule_all_then assume_tac ffv_fabs >>
...
>- (drule_at_then Any assume_tac abssl_id >> gs[]) >>
Cases_on ‘ok_abs l’ >> gs[]
QED
...
Theorem frename_fVar_subst:
  ∀f.
    ...
    (∀st. MEM st sl ⇒ (n,s) ∉ sfv st) ∧
    (P,sl) ∈ freefVars f ⇒        
    (frename (n,s) nn (fVar_subst (P,sl,ϕ) f)) =
    fVar_subst (P,sl,frename (n,s) nn ϕ) (frename (n,s) nn f)
Proof
  Induct_on ‘f’ >> gs[frename_def,finst_def,ffv_thm,tfv_thm,fVar_subst_def,freefVars_def,
                      PULL_EXISTS] (* 3 *) >> rw[] (* 7 *) >> gs[GSYM frename_def]
  >- metis_tac[]
  >- (Cases_on ‘(P,sl) ∈ freefVars f'’ >- metis_tac[] >>
      ‘(fVar_subst (P,sl,ϕ) f') = f'’
        by metis_tac[fVar_subst_id] >> simp[] >>
      ‘(P,sl) ∉ freefVars (frename (n,s) nn f')’
        suffices_by metis_tac[fVar_subst_id] >>
      ‘ok_abs sl’ by metis_tac[freefVars_ok_abs] >>
      irule NOTIN_freefVars_frename >> simp[] >> rw[] 
      >> metis_tac[])
  >- (Cases_on ‘(P,sl) ∈ freefVars f’ >- metis_tac[] >>
      ‘(fVar_subst (P,sl,ϕ) f) = f’
        by metis_tac[fVar_subst_id] >> simp[] >>
  ...
  >- metis_tac[]
  >- metis_tac[]
  >- (rw[GSYM rich_listTheory.MAP_REVERSE] >>
      rw[GSYM trename_def,mk_bmap_MAP] >>
      metis_tac[frename_fprpl]) >>
  ‘l = MAP (sinst (TO_FMAP [((n,s),Var nn s)])) l’
    suffices_by metis_tac[] >>
...
>- (‘(P,sl) ∉ freefVars (abst (n,s) f)’
       by metis_tac[freefVars_fabs_SUBSET,
                    abst_def,SUBSET_DEF] >>
    drule_then assume_tac fVar_subst_id >> simp[] >>
...
>- (Cases_on ‘∃st. MEM st sl ∧ (n,s) ∈ sfv st’ (* 2 *)
   >- (‘(P,sl) ∉ freefVars (abst (n,s) f)’
        by (drule_then assume_tac freefVars_fabs >>
           simp[abst_def]) >>
       drule_then assume_tac fVar_subst_id >> simp[] >>
       rw[GSYM mk_FALL_def] >> irule $ cj 5 wff_rules >>
       simp[] >> metis_tac[]) >>
   ‘(P,sl) ∈ freefVars (abst (n,s) f)’
    by (drule_then assume_tac freefVars_fabs >>
           simp[abst_def]) >>
   reverse (Cases_on ‘(n,s) ∈ ffv ϕ’) >-
   (‘∀st. MEM st sl ⇒ (n,s) ∉ sfv st’ by metis_tac[] >>
   rev_drule_all_then assume_tac fVar_subst_fabs1 >>
   rw[abst_def] >> simp[] >>
   rw[GSYM mk_FALL_def,GSYM abst_def] >>
   irule $ cj 5 wff_rules >>
   simp[] >>
   rev_drule_at_then Any assume_tac ffv_fVar_subst >>
   ‘(∀n s. (n,s) ∈ ffv ϕ ⇒ sbounds s = ∅)’
    by metis_tac[wff_FALLL_no_vbound]>> 
   ...
   metis_tac[]) >>
   ‘∃nn.
    (nn,s) ∉ ffv f ∪ ffv ϕ ∪ BIGUNION {sfv st | MEM st sl} ∪
     sfv s’
    by
    (‘FINITE (ffv f ∪ ffv ϕ ∪ BIGUNION {sfv st | MEM st sl} ∪ sfv s)’ by (simp[ffv_FINITE,tfv_FINITE] >>
           rw[] (* 2 *)
           >- simp[FINITE_lemma] >>
           simp[tfv_FINITE]) >>
    drule_then assume_tac fresh_name_ex >>
    metis_tac[]) >>
   ‘fVar_subst (P,sl,ϕ) (abst (n,s) f) =
    (frename (nn,s) n
    (abst (n,s) (fVar_subst (P,sl,frename (n,s) nn ϕ) f)))’
    by (rw[abst_def] >>
       irule fVar_subst_fabs >> simp[] >>
       ‘nn ≠ n’ by (gs[] >> metis_tac[]) >>
       ‘(nn,s) ∉ ffv ϕ’ by gs[] >> simp[] >>
       ‘(nn,s) ∉ ffv f’ by gs[] >> simp[] >>
       gs[] >> metis_tac[]) >> simp[] >>
   ‘wff (Σf,Σp) (frename (n,s) nn (FALLL sl ϕ))’
    by (irule wff_frename >> simp[]) >>
   ‘(frename (n,s) nn (FALLL sl ϕ)) =
    (FALLL sl (frename (n,s) nn ϕ))’
    by (simp[frename_FALLL] >>
       ‘(MAP (srename (n,s) nn) sl) = sl’
        suffices_by metis_tac[] >>
       rw[MAP_fix] >> metis_tac[trename_fix]) >> 
   gs[] >>
   first_x_assum $ drule_then assume_tac >>
   ‘(FALL s
             (frename (nn,s) n
    ...
    (FALL (srename (nn,s) n s)
             (frename (nn,s) n
                (abst (n,s) (fVar_subst (P,sl,frename (n,s) nn ϕ) f))))’
     by (rw[] >> metis_tac[trename_fix]) >>
   simp[] >> 
   rw[Once frename_def,Once srename_def,Once (GSYM finst_def)] >>
   ‘(FALL (sinst (TO_FMAP [((nn,s),Var n s)]) s)
             (finst (TO_FMAP [((nn,s),Var n s)])
                (abst (n,s) (fVar_subst (P,sl,frename (n,s) nn ϕ) f)))) =
    finst (TO_FMAP [((nn,s),Var n s)])
     (FALL s (abst (n,s) (fVar_subst (P,sl,frename (n,s) nn ϕ) f)))’ by simp[finst_def] >>
   pop_assum SUBST_ALL_TAC >>
   rw[GSYM frename_def] >>
   ‘wff (Σf,Σp)
    (FALL s (abst (n,s) (fVar_subst (P,sl,frename (n,s) nn ϕ) f)))’
    by (rw[GSYM mk_FALL_def] >> irule $ cj 5 wff_rules >>
       simp[] >> rw[] >>
       ‘ffv (fVar_subst (P,sl,frename (n,s) nn ϕ) f) ⊆ ffv f ∪ ffv (frename (n,s) nn ϕ)’
        by (irule ffv_fVar_subst >> simp[] >>
           rw[] >> CCONTR_TAC >>
           ‘∃i. i ∈ sbounds s'’
             by metis_tac[MEMBER_NOT_EMPTY] >>
           ‘i ∈ BIGUNION (IMAGE (sbounds o SND)
                (ffv (frename (n,s) nn ϕ)))’
              by (rw[IMAGE_DEF,PULL_EXISTS] >>
                 qexists ‘(n',s')’ >> simp[]) >>
           ‘BIGUNION (IMAGE (sbounds ∘ SND) (ffv (frename (n,s) nn ϕ))) = BIGUNION (IMAGE (sbounds ∘ SND) (ffv ϕ))’
            by metis_tac[BIGUNION_IMAGE_sbounds_ffv] >>
           ‘BIGUNION (IMAGE (sbounds ∘ SND) (ffv ϕ)) = {}’
            suffices_by (strip_tac >> gs[] >> gs[EXTENSION]>>
             metis_tac[]) >>
           simp[Once EXTENSION] >>
           Cases_on ‘ (∀x. x ∉ ffv ϕ)’ >> simp[] >>
           simp[Once EXTENSION] >>
           rw[] >>
           rw[EQ_IMP_THM] (* 2 *)
           >- (Cases_on ‘x'’ >> simp[] >>
              ...
              metis_tac[wff_no_vbound]) >>
           gs[] >>
           Cases_on ‘x'’ >> qexists ‘(q,r)’ >> simp[] >>
            ‘(q,r) ∈ ffv (FALLL sl ϕ)’
                by simp[ffv_FALLL] >>
              metis_tac[wff_no_vbound]) >>
       ‘(n1,s1) ∈ ffv f ∪ ffv (frename (n,s) nn ϕ)’
        by gs[SUBSET_DEF] >>
       gs[] (* 2 *) >- metis_tac[] >>
       ‘(n,s) ∉ ffv (frename (n,s) nn ϕ)’
        suffices_by metis_tac[sfv_ffv,SUBSET_DEF] >>
       irule NOTIN_frename >> metis_tac[]) >>
