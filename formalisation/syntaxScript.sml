...
>- (rename [‘(n1,st1) ∉ FDOM σ’] >> 
   ...
   >- (gs[] >>  irule ssubtm_tsubtm >>  gs[no_bound_def] >>
       gs[cstt_def] >>
       first_x_assum irule >> simp[] >>
       gs[tbounds_def]) >>
   gs[tsubtm_def] >> disj2_tac >> first_x_assum irule >> simp[] >>
    gs[tbounds_def])
>- (gs[tsubtm_def,MEM_MAP,PULL_EXISTS] >>
   rpt disj2_tac >> qexists ‘t’ >> simp[] >>
   first_x_assum irule >> simp[] >> gs[tbounds_Fn])
>- gs[tsubtm_def,MEM_MAP,PULL_EXISTS,tbounds_Fn] >>
gs[tsubtm_def,MEM_MAP,PULL_EXISTS,sbounds_St] >>
metis_tac[]
QED
...
Theorem sfv_tfv:
∀t n s. tbounds t = {} ∧ (n,s) ∈ sfv (sort_of t) ⇒ (n,s) ∈ tfv t
Proof
Cases_on ‘t’ >> gs[tbounds_def,tbounds_Fn,sbounds_St,sort_of_def,tfv_def]
QED



Theorem tfv_sinst:
(∀t σ. cstt σ ∧ tfv t ⊆ FDOM σ ∧ no_bound σ ∧ tbounds t = {}  ⇒
 (∀n st. (n,st) ∈ tfv (tinst σ t) ⇔
       ∃n0 st0. (n0,st0) ∈ tfv t ∧ (n,st) ∈ tfv (σ ' (n0,st0)))) ∧
(∀s σ. cstt σ ∧ sfv s ⊆ FDOM σ ∧ no_bound σ ∧ sbounds s = {} ⇒
...
>- (eq_tac (* 2 *)
   ...
   irule sfv_tfv >> gs[no_bound_def])
>- (eq_tac(* 2 *)
   >- (simp[PULL_EXISTS,MEM_MAP] >> gs[tbounds_Fn] >> rw[] (* 2 *)
      ...
      gs[] >> metis_tac[]) >>
   simp[PULL_EXISTS,MEM_MAP] >> gs[tbounds_Fn] >>
   rw[] (* 2 *)
   >- (disj1_tac >> qexists ‘t’ >> simp[] >>
      first_x_assum (irule o iffRL) >> simp[] >> gs[SUBSET_DEF] >>
      metis_tac[]) >>
   disj2_tac >> metis_tac[]) >>
simp[PULL_EXISTS,MEM_MAP] >> gs[sbounds_St] >>  eq_tac (* 2 *)
