...
Theorem wff_fprpl_mk_bmap:
wffVsl Σf sl ∧ wff (Σf,Σp,Σe) (FALLL sl b) ⇒
∀n. n < LENGTH sl ⇒
            is_EQ (concl (eqths n)) ∧ Pf0 (Σf,Σp,Σe) aths (Pf0s n) ∧
            (∀Γ A f.
               MEM (Γ,A,f) (Pf0s n) ⇒
               Uof ffv ({f} ∪ A) ⊆ Γ ∧ wff (Σf,Σp,Σe) f ∧
               ∀a. a ∈ A ⇒ wff (Σf,Σp,Σe) a) ∧ MEM (eqths n) (Pf0s n) ∧
            sort_of (Leq (concl (eqths n))) = EL n sl
...
>~ [‘(Γ,A,f) = fcong (map2list (LENGTH sl − 1) eqths) sl b’]
   ...
   (* fVinst_def *)
cheat (* need to realise as fVinst *)*) cheat
>~ [‘(Γ,A,f) = fcong (map2list (LENGTH sl − 1) eqths) sl b’]
   >- (gs[fcong_def,IN_Uof,MEM_map2list] >>
    ‘LENGTH sl ≠ 0’ by simp[] >>
    ‘n0 < LENGTH sl’ by simp[] >>
...
sl ≠ [] ∧ wffVsl (FST Σ) sl ∧
       (∀n. n < LENGTH sl ⇒
            ...
            sort_of (Leq (concl (eqths n))) = EL n sl) ∧
       wfabsap (FST Σ) sl (Lofeqthl (map2list (LENGTH sl - 1) eqths)) ∧
       wfabsap (FST Σ) sl (Rofeqthl (map2list (LENGTH sl - 1) eqths)) ∧ wff Σ (FALLL sl b) ∧ is_cfm b ⇒
       Pf0Drv Σ aths
         (fcong (map2list (LENGTH sl − 1) eqths) sl b)
Proof
rw[Pf0Drv_def] >>
