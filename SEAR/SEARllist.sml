...
e0
(rpt strip_tac >>
accept_tac (IN_def_P |> qtinst_thm [(‘A’,‘Pow(A)’)]
|> qfVar_prpl_th1 ("P",[‘sa:mem(Pow(A))’],‘SS(sa,App(f:Pow(A) -> Pow(A),sa))’)))
...
(rpt strip_tac >>
 ...
 |> qfVar_prpl_th1 ("P",[‘n:mem(N)’,‘x1:mem(X+1)’],
...
(qsuff_tac
 ...
 assume_tac
 (P2fun' |> qtinst_thm [(‘A’,‘Pow(Exp(N,X+1))’),(‘B’,‘Pow(Exp(N,X+1))’)]
         |> qfVar_prpl_th1 ("P",[‘s0:mem(Pow(Exp(N,X+1)))’,‘s1:mem(Pow(Exp(N,X+1)))’],‘!g. IN(g,s1) <=> 
         g = Tpm(Null(X)) |
 ...
 ‘!s0. ?!y. !g. IN(g,y) <=> 
  ...
  ?h t. g  = Tpm(lcons0(h,t)) & IN(Tpm(t),s0:mem(Pow(Exp(N,X+1))))’
 >-- (strip_tac >> 
 assume_tac (IN_def_P |> qtinst_thm [(‘A’,‘Exp(N,X+1)’)]
                      |> qfVar_prpl_th1 ("P",[‘g:mem(Exp(N,X+1))’],‘g = Tpm(Null(X)) |
  ?h t. g  = Tpm(lcons0(h,t)) & IN(Tpm(t),s0:mem(Pow(Exp(N,X+1))))’)) >>
 arw[]) >>
 first_x_assum drule >>
...
val llist_def1 = Thm_2_4' |> qtinst_thm [(‘A’,‘Exp(N,X+1)’)]
                         |> qfVar_prpl_th1 ("P",[‘g:mem(Exp(N,X+1))’],
‘IN(g,islls(X))’)
...
(strip_tac >> 
 ...
 >-- (irule (P2fun' |> qtinst_thm [(‘A’,‘Exp(X,X)’),(‘B’,‘Exp(X,X)’)] 
 |> qfVar_prpl_th1 ("P",[‘g1:mem(Exp(X,X))’,‘g2:mem(Exp(X,X))’],
‘g2 = Tpm(tof(g1:mem(Exp(X,X))) o f:X->X)’)) >>
 ...
 >-- (irule (P2fun' |> qtinst_thm [(‘A’,‘N * X’),(‘B’,‘X’)] 
                    |> qfVar_prpl_th1 ("P",[‘nx:mem(N*X)’,‘x1:mem(X)’],
...
(rpt strip_tac >>
 ...
 >-- (strip_tac >> uex_tac >> qexists_tac ‘ob’ >> arw[]>> rpt strip_tac >>
     ...
     fs[Tpm_tof_inv]) >>
 assume_tac
 (P2fun' |> qtinst_thm [(‘A’,‘(A + 1) * Exp(A,B+1)’),(‘B’,‘B + 1’)]
         |> qfVar_prpl_th1 ("P",[‘a1f:mem((A + 1) * Exp(A,B+1))’,‘b1:mem(B + 1)’],‘(Fst(a1f:mem((A + 1) * Exp(A,B+1))) = NONE(A) & b1 = NONE(B)) |
...
(strip_tac >> 
 ...
 >-- (strip_tac >> uex_tac >> qexists_tac ‘fpb’ >> arw[] >>
     ...
     Pair_has_comp >> arw[]) >> 
 assume_tac (P2fun' |> qtinst_thm [(‘A’,‘(B * A) +1’),(‘B’,‘(B * A) + 1’)] 
                    |> qfVar_prpl_th1 ("P",[‘ba1:mem((B * A) + 1)’,‘ba2:mem((B * A) + 1)’],‘(ba1 = NONE(B * A) & ba2 = NONE(B * A)) |
...
(rpt strip_tac >>
 ...
 irule
 (P2fun' |> qtinst_thm [(‘A’,‘N’),(‘B’,‘A + 1’)]
         |> qfVar_prpl_th1 ("P",[‘n:mem(N)’,‘a1:mem(A+1)’],‘a1 =  App(OM(p2(B,A)),App(FP(FPB(f)),Pair(n,App(f:B -> (B * A)+1,z))))’)) >>
 strip_tac >> uex_tac >>
...
qby_tac
 ‘?sa. !g.IN(g,sa)<=>
   ?z.g = Tpm(toabs(f,z))’ >-- 
accept_tac (IN_def_P_ex |> qtinst_thm [(‘A’,‘Exp(N,A+1)’)]
                        |> qfVar_prpl_th1 ("P",[‘g:mem(Exp(N, A + 1))’],
...
(rpt strip_tac >>
 ...
 >-- (strip_tac >> uex_tac >> qexists_tac ‘g’ >> arw[] >>
     rpt strip_tac >> rw[GSYM FUN_EXT] >> arw[]) >>
 irule
 (P2fun' |> qtinst_thm [(‘A’,‘B’),(‘B’,‘Exp(N,A + 1)’)] 
         |> qfVar_prpl_th1 ("P",[‘b:mem(B)’,‘f0:mem(Exp(N,A+1))’],
...
(strip_tac >>
 ...
 >-- (strip_tac >> uex_tac >> qexists_tac ‘cB’ >> arw[] >> rpt strip_tac >>
     ...
     Pair_has_comp >> arw[]) >>
 assume_tac (P2fun' |> qtinst_thm [(‘A’,‘Pow(llist(X) * llist(X))’),
                               (‘B’,‘Pow(llist(X) * llist(X))’)]
                    |> qfVar_prpl_th1 ("P",[‘R0:mem(Pow(llist(X) * llist(X)))’,‘R1:mem(Pow(llist(X) * llist(X)))’],‘!ll1 ll2.IN(Pair(ll1,ll2),R1) <=> 
                    (ll1 = LNil(X) & ll2 = LNil(X)) | 
 ...
 >-- (strip_tac >>
     assume_tac 
     (IN_def_P |> qtinst_thm [(‘A’,‘llist(X) * llist(X)’)] 
               |> qfVar_prpl_th1 ("P",[‘ll12:mem(llist(X) * llist(X))’],
...
(rpt strip_tac >> dimp_tac >> strip_tac >> arw[] >-- 
 ...
 >-- assume_tac (IN_def_P |> qtinst_thm [(‘A’,‘llist(X) * llist(X)’)] 
 |> qfVar_prpl_th1 ("P",[‘a:mem(llist(X) * llist(X))’],
 ‘Fst(a) = Snd(a:mem(llist(X) * llist(X)))’)
              |> uex2ex_rule
                 |> conv_rule(depth_fconv no_conv forall_cross_fconv)
                 |> rewr_rule[Pair_def']) >>
...
val lmapf_def =
    P2fun_uex
    |> qtinst_thm [(‘A’,‘llist(X)’),(‘B’,‘llist(X) * A + 1’)] 
    |> qfVar_prpl_th1 ("P",[‘l:mem(llist(X))’,‘opv:mem(llist(X) * A +1)’],
      ‘(l = LNil(X) & opv = NONE(llist(X) * A)) |
     ?lh lt. l = LCons(lh,lt) & opv = SOME(Pair(lt,App(f:X->A,lh)))’)
    |> C mp f_ex0 
...
(rpt strip_tac >> once_rw[LLIST_BISIMULATION0] >>
 strip_assume_tac
 (IN_def_P |> qtinst_thm [(‘A’,‘llist(Y) * llist(Y)’)]
           |> qfVar_prpl_th1 ("P",[‘ll12:mem(llist(Y) * llist(Y))’],
