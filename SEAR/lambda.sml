...
e0
(dimp_tac >> strip_tac >> qcases ‘A’ >> qcases ‘B’ >> 
 arw[] >> first_x_assum opposite_tac)
(form_goal “(A | (~A & B)) <=> A | B”)
...
(form_goal “!a:mem(A). P[a:mem(A)](a) ==> !B b:mem(B).?!b'. P[a:mem(A)](a) & b = b'”)
...
fun define_lambda_fun f = 
    let val (inputvar as (n,s),clauses) = dest_forall f
        ...
        val culemma = cond_unique_lemma' |> inst_thm
 (mk_inst [(("A",set_sort),setvar)] []) |> specl [inputt]
        val fvar0 = mk_fvar "P" [(n,s)] [inputt]
        val cases = List.map 
                        (fn (cond,output) =>
                            culemma 
                                |> fVar_prpl_th
        ...
        val fvar1 = mk_fvar "P" [(n,s),outputv] [inputt,outputt]
        val specp2fun = P2fun_uex |> inst_thm 
                                  (mk_inst [(("A",set_sort),setvar),
                                            (("B",set_sort),outputset)] [])
                                  |> fVar_prpl_th
                                  (mk_fd [("P",[(n,s),outputv],rel)])
