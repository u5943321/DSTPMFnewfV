...
val ZR_def = 
AX1 |> qtinst_thm [(‘A’,‘N * N’),(‘B’,‘N * N’)] 
    |> qfVar_prpl_th1
("P",[‘mn:mem(N * N)’,‘m'n':mem(N * N)’],
...
val Ri_def = 
P2fun_uex'|> qtinst_thm [(‘A’,‘Pow(A)’),(‘B’,‘Pow(B)’)]
          |> qfVar_prpl_th1 
("P",[‘sa:mem(Pow(A))’,‘sb:mem(Pow(B))’],
‘!b. IN(b,sb) <=> ∃a. IN(a,sa) & Holds(r:A~>B,a,b)’)
                   |> C mp 
                      (IN_def_P |> qtinst_thm [(‘A’,‘B’)]
                                |> qfVar_prpl_th1
("P",[‘b:mem(B)’],‘∃a. IN(a,sa) & Holds(r:A~>B,a,b)’)
                                |> qgen ‘sa’)
...
val Z_def = Thm_2_4'|> qtinst_thm [(‘A’,‘Pow(N * N)’)]
                    |> qfVar_prpl_th1 
("P",[‘s:mem(Pow(N * N))’],‘?n. s = rsi(ZR,n)’)
...
val rext_def0 =  AX1 |> qtinst_thm [(‘A’,‘Pow(A)’),(‘B’,‘Pow(B)’)]
                     |> qfVar_prpl_th1 
("P",[‘sa:mem(Pow(A))’,‘sb:mem(Pow(B))’],
...
val prrel_def = AX1 |> qtinst_thm [(‘A’,‘A * B’),(‘B’,‘A*B’)]
                    |> qfVar_prpl_th1
("P",[‘ab1:mem(A*B)’,‘ab2:mem(A*B)’],
...
(rpt strip_tac >> assume_tac 
 (P2fun' |> qtinst_thm [(‘A’,‘Q1’),(‘B’,‘Q2’)]
         |> qfVar_prpl_th1 ("P",[‘q1:mem(Q1)’,‘q2:mem(Q2)’],
...
val ipow2_def = P2fun_uex' 
                    |> qtinst_thm [(‘A’,‘Q1 * Q2’),
                                   (‘B’,‘Pow(A*B)’)]
                    |> qfVar_prpl_th1
                       ("P",[‘aqbq:mem(Q1 * Q2)’,‘s:mem(Pow(A * B))’],‘!a1 a2.IN(Pair(a1,a2),s:mem(Pow(A * B))) <=> 
                         IN(a1,App(i1:Q1-> Pow(A),Fst(aqbq))) & 
                         IN(a2,App(i2:Q2-> Pow(B),Snd(aqbq)))’)
                     |> C mp 
                     (IN_def_P |> qtinst_thm [(‘A’,‘A * B’)]
                               |> qfVar_prpl_th1
("P",[‘ab:mem(A * B)’],‘IN(Fst(ab),App(i1:Q1->Pow(A),Fst(aqbq))) & 
...
local 
val l = P2fun_uex' |> qtinst_thm [(‘A’,‘(N * N) * N * N’),
(‘B’,‘N * N’)]
       |> qfVar_prpl_th1 
       ("P",[‘xyuv:mem((N * N) * N * N)’,‘ab:mem(N * N)’],
...
e0
(rpt strip_tac >> assume_tac 
 (P2fun_uex|> qtinst_thm [(‘A’,‘Q1’),(‘B’,‘Q2’)]
        |> qfVar_prpl_th1 ("P",[‘q1:mem(Q1)’,‘q2:mem(Q2)’],
‘Holds(rext(f:A->B, r1, r2), 
                      App(i1:Q1->Pow(A), q1), 
                      App(i2:Q2->Pow(B), q2))’)) >>
...
local 
val l = P2fun_uex' |> qtinst_thm [(‘A’,‘(N * N) * N * N’),(‘B’,‘N * N’)]
       |> qfVar_prpl_th1 ("P",[‘abcd:mem((N * N) * N * N)’,
‘mn:mem(N * N)’],‘mn  = Pair(Add(Mul(Fst(Fst(abcd)),Fst(Snd(abcd))),
...
val absz_def = 
    P2fun_uex0
        |> qtinst_thm [(‘A’,‘N * N’),(‘B’,‘Z’)]
        |> qfVar_prpl_th1 ("P",[‘ab:mem(N*N)’,‘y:mem(Z)’],
‘Repz(y) = ZC(ab)’)
...
(dimp_tac >> rpt strip_tac (* 2 *)
 >-- (qspecl_then [‘z’] strip_assume_tac cases_z >>
     arw[]) >>
 arw[])
(form_goal “(!a b. P[z:mem(Z)](Asz(a,b))) <=> (!z:mem(Z). P[z:mem(Z)](z))”));
...
val LEz_def = AX1 |> qtinst_thm [(‘A’,‘Z’),(‘B’,‘Z’)]
                  |> qfVar_prpl_th1 ("P",[‘a:mem(Z)’,‘b:mem(Z)’],‘Lez(a,b)’)  |> qsimple_uex_spec "LEz" []
                  |> store_as "LEz_def";
...
(rpt strip_tac >> x_choosel_then ["s"] strip_assume_tac
 (IN_def_P_ex |> qtinst_thm [(‘A’,‘N’)] |> GSYM
 |> qfVar_prpl_th1 ("P",[‘n:mem(N)’],
...
val DIVRz_def = 
P2fun_uex |> qtinst_thm [(‘A’,‘Z * Z’),(‘B’,‘Z*Z’)]
|> qfVar_prpl_th1 ("P",[‘ad:mem(Z*Z)’,‘qr:mem(Z*Z)’],
...
val DIVR_def = 
P2fun_uex |> qtinst_thm [(‘A’,‘N * N’),(‘B’,‘N*N’)]
          |> qfVar_prpl_th1 ("P",[‘ad:mem(N*N)’,‘qr:mem(N*N)’],‘(Snd(ad) = O & qr = Pair(O,O)) |
  (~(Snd(ad) = O) &
