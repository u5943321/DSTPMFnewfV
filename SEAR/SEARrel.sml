...
“?!R:A~>B.!a:mem(A) b:mem(B).Holds(R,a,b)<=> P[a:mem(A),b:mem(B)](a,b)”);


local
val l = AX1 |> uex_expand
            |> qfVar_prpl_th [("P",[‘a:mem(A)’,‘b:mem(B)’],‘Holds(R1:A~>B,a,b)’)] 
in
...
(*parser bug,
 ?R:A~>B. !a b. Holds(R,a,b) <=> P[x:mem(A),y:mem(B)](a,b)
not work
*)
...
e0
(rpt strip_tac >>
 (strip_assume_tac o uex_expand) AX1 >>
 qexists_tac ‘R’ >> 
 rpt strip_tac >> arw[])
(form_goal
 “?R:A~>B. !a:mem(A) b:mem(B). Holds(R,a:mem(A),b:mem(B)) <=> P[a:mem(A),b:mem(B)](a,b)”));

val ao_uex = 
AX1 |> inst_thm (mk_menv (mk_tenv [(("B",set_sort),rastt "C")]) (mk_fenv []))
    |>  qfVar_prpl_th [("P",[‘a:mem(A)’,‘b:mem(C)’],‘(?b1. Holds(phi:A~>B,a,b1) & Holds(psi:B~>C,b1,b))’)]
...
val id_uex =
    AX1 |> inst_thm 
        (qmk_tinst [(‘B’,‘A’)])
        |> 
qfVar_prpl_th [("P",[‘a1:mem(A)’,‘a2:mem(A)’],‘a1 = a2:mem(A)’)]
...
(rpt strip_tac >> once_rw[GSYM R_EXT] >> rw[GSYM ao_def] >> rpt strip_tac >>
 ...
 >-- (qexists_tac ‘b1'’ >> arw[] >> qexists_tac ‘b1’ >> arw[]) >>
 qexists_tac ‘b1'’ >> arw[] >> qexists_tac ‘b1’ >> arw[])
(form_goal
...
val op_uex = 
    AX1 |> inst_thm (qmk_tinst [(‘A’,‘B’),(‘B’,‘A’)])
        |> qfVar_prpl_th [("P",[‘b:mem(B)’,‘a:mem(A)’],‘Holds(R:A~>B,a,b)’)]
