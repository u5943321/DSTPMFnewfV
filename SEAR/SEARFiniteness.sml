...
(x_choose_then "s" (assume_tac o conjE1) 
 (IN_def_P_expand |> qtinst_thm [(‘A’,‘Pow(X) * N’)]) >>
 arw[Cds_ind])
(form_goal 
 “P[xsn:mem(Pow(X) * N)](Pair(Empty(X),O)) & 
 (!xs0 n0 x:mem(X). P[xsn:mem(Pow(X) * N)](Pair(xs0,n0)) & ~(IN(x,xs0)) ==> 
  P[xsn:mem(Pow(X) * N)](Pair(Ins(x,xs0),Suc(n0)))) ==> 
 !xs n. IN(Pair(xs,n),Cds(X)) ==> P[xsn:mem(Pow(X) * N)](Pair(xs,n))”));


val Cd_induct = prove_store("Cd_induct",
e0
(assume_tac (Cd_induct0 
            |> qfVar_prpl_th1 ("P",[‘xsn:mem(Pow(X) * N)’],‘P[xs:mem(Pow(X)),n:mem(N)](Fst(xsn:mem(Pow(X) * N)),Snd(xsn))’)
            |> rewr_rule[Pair_def']) >> arw[])
(form_goal 
 “P[xsn:mem(Pow(X)),n:mem(N)](Empty(X),O) & 
 (!xs0 n0 x:mem(X). P[xsn:mem(Pow(X)),n:mem(N)](xs0,n0) & ~(IN(x,xs0)) ==> 
  P[xsn:mem(Pow(X)),n:mem(N)](Ins(x,xs0),Suc(n0))) ==> 
 !xs n. IN(Pair(xs,n),Cds(X)) ==> P[xsn:mem(Pow(X)),n:mem(N)](xs,n)”));
 
val Fin_induct = prove_store("Fin_induct",
e0
(rw[Fin_def] >> 
 x_choose_then "s" (assume_tac o conjE1) 
 (IN_def_P_expand |> qtinst_thm [(‘A’,‘Pow(X)’)] ) >>
 arw[FI_ind])
(form_goal 
 “P[xs:mem(Pow(X))](Empty(X)) & 
 (!xs0:mem(Pow(X)) x. P[xs:mem(Pow(X))](xs0) ==> P[xs:mem(Pow(X))](Ins(x,xs0))) ==> 
 !xs:mem(Pow(X)). Fin(xs) ==> P[xs:mem(Pow(X))](xs)”));
...
e0
(strip_tac >> ind_with (Fin_induct) >> rpt strip_tac (* 2 *)
...
val CARD_def = 
    AX1 |> qtinst_thm [(‘A’,‘Pow(X)’),(‘B’,‘N’)]
        |> qfVar_prpl_th1 ("P",[‘xs:mem(Pow(X))’,‘n:mem(N)’],‘(Fin(xs:mem(Pow(X))) & Cdr(xs,n)) | (~Fin(xs) & n = O)’)
...
val Cd0_def = P2fun_uex0 |> qtinst_thm [(‘A’,‘Pow(X)’),(‘B’,‘N’)]
                 |> qfVar_prpl_th1 ("P",[‘x:mem(Pow(X))’,‘y:mem(N)’],‘Holds(CARD(X),x,y)’)       |> C mp (CARD_unique |> qspecl [‘X’]) 
...
e0
(strip_tac >>
 ind_with (Fin_induct |> qtinst_thm [(‘X’,‘A’)]) >>
