...
val restrict_def = 
    AX1 
        |> qfVar_prpl_th1 ("P",[‘a:mem(A)’,‘b:mem(B)’],
...
(rpt strip_tac >>
 ...
 drule  (P2fun |> qtinst_thm [(‘A’,‘B’),(‘B’,‘A’)]
               |> qfVar_prpl_th1 ("P",[‘b:mem(B)’,‘a:mem(A)’],‘IN(a,s) & Holds(R:A~>B,a,b)’)) >>
...
val POW_def = IN_def_P |> qtinst_thm [(‘A’,‘Pow(A)’)] 
                       |> qfVar_prpl_th1
                       ("P",[‘s:mem(Pow(A))’],‘SS(s,s0:mem(Pow(A)))’)               |> qsimple_uex_spec "POW" [‘s0’]
                       |> gen_all
...
(rpt strip_tac >>
 ...
 >-- (strip_tac >>
     ...
     >-- (qexists_tac ‘a’ >> arw[]) >> rfs[]) >>
 drule
 (P2fun |> qtinst_thm [(‘A’,‘Pow(A)’)] 
 |> qfVar_prpl_th1 ("P",[‘s:mem(Pow(A))’,‘b:mem(B)’],
...
(rpt strip_tac >>
 ...
 >-- (rpt strip_tac >> uex_tac >> qexists_tac ‘f’ >>
     ...
     first_x_assum drule >> first_x_assum drule >> fs[]) >>
 strip_assume_tac
 (P2fun |> qtinst_thm [(‘A’,‘Pow(X)’),(‘B’,‘X’)] 
 |> qfVar_prpl_th1 ("P",[‘s:mem(Pow(X))’,‘x:mem(X)’],
‘s = Sing(x) |
...
(rpt strip_tac >> fs[cardeq_biunique] >>
 ...
 (AX1 |> qtinst_thm [(‘A’,‘Pow(A)’),(‘B’,‘Pow(B)’)]
      |> qfVar_prpl_th1 ("P",[‘s:mem(Pow(A))’,‘t:mem(Pow(B))’],‘SS(s,s1) & SS(t,s2) & t = App(Ri(restrict(R:A~>B,s1,s2)),s)’)) >>
...
(rpt strip_tac >>
 fs[nPow_def] >>
 strip_assume_tac
 (IN_def_P_ex |> qtinst_thm [(‘A’,‘N’)] 
 |> qfVar_prpl_th1 ("P",[‘n0:mem(N)’],‘Le(n0,n)’)
 |> GSYM) >>
...
val m2s_def = 
Thm_2_4' |> qfVar_prpl_th1 ("P",[‘a:mem(A)’],‘IN(a,s:mem(Pow(A)))’)   |> set_spec (rastt "A") "m2s" "minc" 
...
(rpt strip_tac >>
 ...
 (AX1 |> qtinst_thm [(‘B’,‘m2s(s:mem(Pow(A)))’)]
 |> qfVar_prpl_th1 ("P",[‘a:mem(A)’,‘b:mem(m2s(s:mem(Pow(A))))’],‘App(minc(s):m2s(s:mem(Pow(A)))->A,b) = a’)) >>
...
(rpt strip_tac >>
 assume_tac
 (AX5 |> qtinst_thm [(‘A’,‘N’)] 
 |> qfVar_prpl_th1 ("P",[‘n:mem(N)’,‘An’],‘nPow(n,A,An)’)) >>
