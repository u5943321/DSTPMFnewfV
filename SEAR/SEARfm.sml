...
e0
(strip_tac >> irule
 (P2fun_uex |> qtinst_thm [(‘A’,‘N’),(‘B’,‘Pow(N * (A+1))’)]
 |> qfVar_prpl_th1 ("P",[‘n0:mem(N)’,‘na:mem(Pow(N*(A+1)))’],‘(!n:mem(N) a:mem(A+1).IN(Pair(n,a),na) <=> n = n0)’)) >>
 strip_tac >> accept_tac
 (IN_def_P |> qtinst_thm [(‘A’,‘N * (A+1)’)]
           |> qfVar_prpl_th1 ("P",[‘na:mem(N * (A+1))’],‘Fst(na:mem(N * (A+1))) = x’) 
...
e0
(strip_tac >> irule
 (P2fun_uex |> qtinst_thm [(‘B’,‘Pow(N * (A+1))’)]
 |> qfVar_prpl_th1 ("P",[‘a0:mem(A)’,‘na:mem(Pow(N * (A+1)))’],
    ‘(!n:mem(N) a:mem(A+1).IN(Pair(n,a),na) <=> a = SOME(a0))’)) >>
 strip_tac >> accept_tac
 (IN_def_P |> qtinst_thm [(‘A’,‘N * (A+1)’)]
 |> qfVar_prpl_th1 ("P",[‘na:mem(N*(A+1))’],‘Snd(na:mem(N * (A+1))) = SOME(x)’)
 |> conv_rule (depth_fconv no_conv forall_cross_fconv)
 |> rewr_rule[Pair_def']))
...
e0
(strip_tac >> irule
 (P2fun_uex |> qtinst_thm [(‘A’,‘Pow(N * A) * Pow(N * A)’),(‘B’,‘Pow(N * A)’)]
 |> qfVar_prpl_th1 ("P",[‘u12:mem(Pow(N * A) * Pow(N * A))’,‘s:mem(Pow(N * A))’], ‘∀n a. IN(Pair(n,a),s:mem(Pow(N * A))) ⇔ 
    ((Even(n) & IN(Pair(Div2(n),a),Fst(u12))) | 
     (Odd(n) & IN(Pair(Div2(n),a),Snd(u12))))’)
 |> conv_rule (depth_fconv no_conv forall_cross_fconv)
 |> rewr_rule[Pair_def']) >>
 rpt strip_tac >> accept_tac
 (IN_def_P |> qtinst_thm [(‘A’,‘N * A’)]
           |> qfVar_prpl_th1 ("P",[‘na:mem(N * A)’],‘(Even(Fst(na)) & IN(Pair(Div2(Fst(na)),Snd(na)),a:mem(Pow(N * A)))) | 
...
(rw[isfm_def] >>
 ...
 (IN_def_P_expand |> qtinst_thm [(‘A’,‘Pow(N * (A+1))’)]) >> arw[] >>
 rpt strip_tac >> irule isfm_ind >> arw[])
(form_goal 
 “P[f0:mem(Pow(N * (A+1)))](F0(A)) & 
  (!p:mem(A). P[f0:mem(Pow(N * (A+1)))](Var0(p))) & 
  (!f0:mem(Pow(N * (A+1))). P[f0:mem(Pow(N * (A+1)))](f0) ==> P[f0:mem(Pow(N * (A+1)))](Neg0(f0))) &
  (!f1:mem(Pow(N * (A+1))) f2. P[f0:mem(Pow(N * (A+1)))](f1) & P[f0:mem(Pow(N * (A+1)))](f2) ==> P[f0:mem(Pow(N * (A+1)))](Disj0(f1,f2))) &
  (!f0:mem(Pow(N * (A+1))). P[f0:mem(Pow(N * (A+1)))](f0) ==> P[f0:mem(Pow(N * (A+1)))](Diam0(f0))) ==>
  !f0:mem(Pow(N * (A+1))). isfm(f0) ==> P[f0:mem(Pow(N * (A+1)))](f0)”));
...
val form_def = Thm_2_4'  |> qtinst_thm [(‘A’,‘Pow(N * (A+1))’)]
                         |> qfVar_prpl_th1 
                         ("P",[‘f:mem(Pow(N * (A+1)))’],‘isfm(f:mem(Pow(N * (A+1))))’)
...
e0
(disch_tac >>
 suffices_tac “!f0:mem(Pow(N * (A+1))). isfm(f0) ==> isfm(f0) &
 !f.Repf(f) = f0 ==> P[f:mem(form(A))](f)” 
 >-- (strip_tac >>
     by_tac “!f0:mem(Pow(N * (A+1))). isfm(f0) ==>
                      (!f. Repf(f) = f0 ==> P[f:mem(form(A))](f))”
     >-- (rpt strip_tac >> first_x_assum drule >>
          rfs[] >> first_x_assum irule >> arw[]) >>
     strip_tac >> first_x_assum irule >>
     qexists_tac ‘Repf(f0)’ >> rw[repf_isfm]) >>
 ind_with isfm_induct >>
...
(form_goal
 “P[f:mem(form(A))](Bot(A)) & 
  (!p:mem(A). P[f:mem(form(A))](Var(p))) & 
  (!f0:mem(form(A)). P[f:mem(form(A))](f0) ==> P[f:mem(form(A))](Neg(f0))) &
  (!f1:mem(form(A)) f2. P[f:mem(form(A))](f1) & P[f:mem(form(A))](f2) ==> P[f:mem(form(A))](Disj(f1,f2))) &
  (!f0:mem(form(A)). P[f:mem(form(A))](f0) ==> P[f:mem(form(A))](Diam(f0))) ==>
  !f0:mem(form(A)). P[f:mem(form(A))](f0)”));
...
(strip_tac >> strip_tac >> strip_tac >> strip_tac >>
 strip_tac >> strip_tac >> 
 ind_with form_induct >> strip_tac (* 2 *)
...
val fmrec_def = P2fun_uex |> qtinst_thm [(‘A’,‘form(A)’),(‘B’,‘X’)]
                          |> qfVar_prpl_th1 ("P",[‘f:mem(form(A))’,‘x:mem(X)’],
‘IN(Pair(f,x),
