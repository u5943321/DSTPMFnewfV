...
val domain_def = 
IN_def_P |> qfVar_prpl_th1 ("P",[‘x:mem(A)’],‘?y. IN(Pair(x,y),r:mem(Pow(A * A)))’)
         |> qsimple_uex_spec "domain" [‘r’]
         |> gen_all 


val range_def = 
IN_def_P |> qfVar_prpl_th1 ("P",[‘y:mem(A)’],‘?x. IN(Pair(x,y),r:mem(Pow(A * A)))’)
...
IN_def_P 
|> qfVar_prpl_th1 ("P",[‘x:mem(A)’],‘IN(x,xs) & 
...
IN_def_P 
|> qfVar_prpl_th1 ("P",[‘x:mem(A)’],‘IN(x,range(r)) & 
    (!y. IN(y,s) ==>
...
(rpt strip_tac >> 
 ...
 strip_assume_tac
 (AX1_ex |>  qtinst_thm [(‘A’,‘Pow(A)’),(‘B’,‘A’)]
     |> qfVar_prpl_th1 ("P",[‘s0:mem(Pow(A))’,‘a0:mem(A)’],
‘(IN(s0,s) & IN(a0,s0)) |
         ~IN(s0,s) & a0 = a:mem(A)’)) >>
 qby_tac
...
IN_def_P
|> qtinst_thm [(‘A’,‘Pow(A)’)]
|> qfVar_prpl_th1 ("P",[‘k:mem(Pow(A))’],‘chain0(k,r) & ~(k = Empty(A)) &
    !C. chain0(C,r) & SS(C,k) &
...
(rpt strip_tac >>
 ...
 (IN_def_P_ex 
 |> qfVar_prpl_th1 ("P",[‘x:mem(A)’],‘IN(x:mem(A),k1) & IN(x,k2) &
     IN(Pair(x,x':mem(A)),r)’)
 |> GSYM) >>
 qby_tac
...
(rpt strip_tac >>
 ...
 assume_tac
(forall_exists_dual
     |> qfVar_prpl_th1 ("P",[‘a:mem(A)’],‘IN(a:mem(A),k1) ==> 
         IN(a,k2)’)) >>
 fs[] >> fs[neg_imp_conj] >>
 assume_tac
(forall_exists_dual
 ...
 >-- (qsuff_tac ‘?y'. IN(y',C) & IN(Pair(y,y'),r)’
     ...
     >-- (fs[upper_bounds_def] >>
         ...
         >-- (rw[range_def] >> 
             ...
             qexists_tac ‘y’ >> rfs[])>>
         fs[] >> fs[] >>
         qby_tac ‘chain0(k,r)’ >-- fs[fchains_def] >>
         qby_tac ‘?y'. IN(y',C) & ~(IN(Pair(y',y),r))’ 
         >-- (ccontra_tac >>  
            qsuff_tac ‘!(y' : mem(A)). IN(y', C) ==> IN(Pair(y', y), r)’ >-- arw[] >>
            qpick_x_assum ‘~(!(y' : mem(A)). IN(y', C) ==> IN(Pair(y', y), r))’ (K all_tac) >>
         ...
         first_x_assum (qsspecl_then [‘y’,‘y'’] assume_tac) >>
         qby_tac ‘IN(y', k)’ >-- (fs[SS_def] >> first_x_assum irule >> arw[]) >>
