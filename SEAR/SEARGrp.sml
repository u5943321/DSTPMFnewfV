...
val Grp_def = Thm_2_4' |> qtinst_thm [(‘A’,‘Exp(G * G,G) * Exp(G,G) * G’)]
                      |> qfVar_prpl_th1 ("P",[‘g:mem(Exp(G * G,G) * Exp(G,G) * G)’],‘isgrp(g:mem(Exp(G * G,G) * Exp(G,G) * G))’)
...
val sgrp_def = Thm_2_4' |> qtinst_thm [(‘A’,‘Pow(G)’)]
                       |> qfVar_prpl_th1("P",[‘H:mem(Pow(G))’],‘issgrp(H:mem(Pow(G)),g)’)
...
(rpt strip_tac >>
 accept_tac
 (IN_def_P |> qtinst_thm [(‘A’,‘G’)]
 |> qfVar_prpl_th1 ("P",[‘x:mem(G)’],‘?h. IN(h,rsg(H:mem(sgrp(g)))) & x = gmul(g:mem(Grp(G)),a,h)’)))
...
(rpt strip_tac >>
 accept_tac
 (IN_def_P |> qtinst_thm [(‘A’,‘G’)]
 |>  qfVar_prpl_th1 ("P",[‘x:mem(G)’],‘?h. IN(h,rsg(H:mem(sgrp(g)))) & x = gmul(g:mem(Grp(G)),h,a)’)))
(form_goal “!G g H:mem(sgrp(g)) a:mem(G). 
...
val nsgrp_def = Thm_2_4' |> qtinst_thm [(‘A’,‘sgrp(g:mem(Grp(G)))’)]
                       |> qfVar_prpl_th1 ("P",[‘H:mem(sgrp(g:mem(Grp(G))))’],
...
(rpt strip_tac >> assume_tac 
 (IN_def_P |> qtinst_thm [(‘A’,‘G’)] 
 |> qfVar_prpl_th1 ("P",[‘a:mem(G)’],‘?y. IN(y,s) & a = gmul(g:mem(Grp(G)),x,y)’)) >> arw[])
...
(rpt strip_tac >> assume_tac 
 (IN_def_P |> qtinst_thm[(‘A’,‘G’)] 
 |> qfVar_prpl_th1 ("P",[‘a:mem(G)’],‘?x. IN(x,s) & a = gmul(g:mem(Grp(G)),x,y)’)) >> arw[])
...
val qgR_def = 
AX1 |> qtinst_thm [(‘A’,‘G’),(‘B’,‘G’)] 
    |> qfVar_prpl_th1 ("P",[‘g1:mem(G)’,‘g2:mem(G)’],
                       ‘lcs(g1,rnsg(H)) =
...
(*cosets*)
val css_def = Thm_2_4' |> qtinst_thm [(‘A’,‘Pow(G)’)]
                    |> qfVar_prpl_th1 ("P",[‘s:mem(Pow(G))’],
                                       ‘?a:mem(G). s = rsi(qgR(H:mem(nsgrp(g))),a)’)                |> set_spec (rastt "Pow(G)") "css" "Rcss" 
...
(rpt strip_tac >> accept_tac
 (IN_def_P |> qtinst_thm [(‘A’,‘G’)] 
 |> qfVar_prpl_th1 ("P",[‘a:mem(G)’],‘?x:mem(G) y. IN(x,s1) & IN(y,s2) & a = gmul(g,x,y)’)))
...
(rpt strip_tac >> accept_tac
 (IN_def_P |> qtinst_thm [(‘A’,‘G’)] 
 |> qfVar_prpl_th1 ("P",[‘a:mem(G)’],‘?x:mem(G). IN(x,s) & a = ginv(g,x)’)))
...
val ghom_def = 
    Thm_2_4' |> qtinst_thm [(‘A’,‘Exp(G1,G2)’)]
            |> qfVar_prpl_th1 ("P",[‘f:mem(Exp(G1,G2))’],‘isghom(tof(f:mem(Exp(G1,G2))),g1,g2)’)
