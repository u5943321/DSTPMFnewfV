...
(form_goal “(!x:mem(A). ?!y:mem(B). P[a:mem(A),b:mem(B)](x,y)) ==>
 ?f:A->B. !a:mem(A) b:mem(B). App(f,a) = b <=> P[a,b](a,b)”));



(*
basic_fconv no_conv (first_fconv (List.map rewr_fconv (rw_fcanon (assume “!a b.P[a:mem(A),b:mem(B)](a,b) <=> b = App(f,a)”)))) “P[a:mem(A),b:mem(B)](a,App(f:A->B,a))”



basic_fconv no_conv (rewr_fconv (spec_all (assume “!a b.P[a:mem(A),b:mem(B)](a,b) <=> b = App(f,a)”))) “P[a:mem(A),b:mem(B)](a,App(f:A->B,a))” 


val th = (spec_all (assume “!a b.P[a:mem(A),b:mem(B)](a,b) <=> b = App(f,a)”))
val partfn = (fst o dest_dimp o concl)

val f = “P[a:mem(A),b:mem(B)](a,App(f:A->B,a))” 

val env = fvd 

(inst_form env) (hd (ant th))

 val f1 = !(a : mem(A))  (b : mem(B)). P(a, b)(a#, b#) <=> b# = App(f, a#):

val f1 = hd (ant th)  
inst_form env f1


(part_fmatch (fst o dest_dimp o concl)  (spec_all (assume “!a b.P[a:mem(A),b:mem(B)](a,b) <=> b = App(f,a)”))) “P[a:mem(A),b:mem(B)](a,App(f:A->B,a))” 
...
val asR_uex =
    AX1 |>  qfVar_prpl_th [("P",[‘a:mem(A)’,‘b:mem(B)’],‘App(f:A->B,a) = b’)] 
val asR_def = 
...
fun fun_tm_compr (ivar as (n,s)) otm = 
    let val isort = s
        ...
        val p = mk_fvar "P" [ivar,dest_var ovt][mk_var ivar,ovt]
        val p1 = mk_eq ovt otm
        val th1 = fVar_prpl_th1 
 ("P",[#2 ivar,(#2 o dest_var) ovt],p1) th0
(*fVar_sInst_th p p1 th0 *)
...
fun fun_tm_compr_uex (ivar as (n,s)) otm = 
    let val isort = s
        ...
        val cod = osort |> dest_sort |> snd |> hd 
        val th0 = P2fun_uex 
                  |> inst_thm (mk_inst [(("A",set_sort),dom),(("B",set_sort),cod)] []) 
        val p =  mk_fvar "P" [ivar,dest_var ovt][mk_var ivar,ovt]
        val p1 = mk_eq ovt otm
        val fd = mk_fd [("P",[ivar,dest_var ovt],p1)]
        val th1 = fVar_prpl_th fd th0
        val lemma = unique_lemma |> allE cod |> allE otm
                                 |> add_cont (fvt (mk_var ivar))
                                 |> allI ivar
        val th2 = mp th1 lemma
...
(form_goal
 ...
 Loading Feedback
> Exception- Fail "pluck: predicate not satisfied" raised

want fix this to tell the exn
...
(*
assume “?f:A->B. !a. P[a:mem(A),b:mem(B)](a,App(f,a))” 
|> qfVar_prpl_th [("P",[‘x:mem(A)’,‘y:mem(B)’],‘
App(g:B->A,y) = x’)] 
...
val qpvsfl = [("P",[‘x:mem(A)’,‘y:mem(B)’],‘
App(f0:A->B,x) = y’)] 

fVar_prpl 0 fd “?f:A->B. !a. P[a:mem(A),b:mem(B)](a,App(f,a))” 


eq_form 
((#1 o strip_forall0)
 (mk_foralls [dest_var (rastt "x:mem(A)"),
dest_var (rastt "y:mem(B)")] “App(f0:A->B,x) = y”),fabsl [dest_var (rastt "x:mem(A)"),
dest_var (rastt "y:mem(B)")] 0 “App(f0:A->B,x) = y”)
...
local 
val l = 
    P2fun' |> inst_thm (qmk_tinst [(‘B’,‘A’)])
           |>  qfVar_prpl_th [("P",[‘x:mem(A)’,‘y:mem(A)’],‘Holds(R:A0~>A0,App(i:A->A0,x),App(i,y))’)] 
in
...
e0
(rpt strip_tac >>
 irule (P2fun'|> inst_thm (qmk_tinst [(‘A’,‘X’),(‘B’,‘A’)])
        |> qfVar_prpl_th [("P",[‘x:mem(X)’,‘a:mem(A)’],‘App(i:A->A0,a) = App(f0:X->A0,x)’)] 
...
(*if use qsuff, then 
  # # Exception- ERR ("anno_ast.ast is a fVar, not a term", [], [], []) raised*)
val P2fun_uex' = prove_store("P2fun_uex'",
e0
(rpt strip_tac >>
 suffices_tac
 “?f:A->B. !a:mem(A). P[x:mem(A),y:mem(B)](a, App(f,a))” 
 >-- (strip_tac >> uex_tac >> qexists_tac ‘f’ >> arw[] >>
 ...
 drule P2fun' >> arw[])
(form_goal “(!x:mem(A). ?!y:mem(B). P[x:mem(A),y:mem(B)](x,y)) ==>
 ?!f:A->B. !a:mem(A). P[x:mem(A),y:mem(B)](a, App(f,a))”));
...
(rpt strip_tac >>
 suffices_tac
 “?f:A->B. !a:mem(A) b:mem(B). App(f,a) = b <=> P[x:mem(A),y:mem(B)](a,b)”
 >-- (strip_tac >> uex_tac >> qexists_tac ‘f’ >> arw[] >>
     rpt strip_tac >> irule $ iffLR FUN_EXT >>
