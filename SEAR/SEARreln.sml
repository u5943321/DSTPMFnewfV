...
fun mk_prim fdef = 
    let val ((pname,psort),b) = fdef |> concl |> dest_forall
        ...
        val fvar0 = mk_fvar "P" [(pname,psort)] [mk_var (pname,psort)]
        ...
        val defname = fnterm |> dest_fun |> #1 |> explode |> rev |> tl 
                             |> rev |> implode
        val spec_IN_ex = IN_def_P
                             |> GSYM 
                             |> inst_thm (mk_inst [(("A",set_sort),pisin)] []) |> GSYM
                             |> fVar_prpl_th (mk_fd [("P",[(pname,psort)],fvar1)])
...
fun mk_fex incond x = 
    let val ((mname,msort),b) = dest_forall incond
        ...
        val tomp = IN_def_P |> inst_thm (mk_inst [(("A",set_sort),misin)] [])
                            |> fVar_prpl_th  
(mk_fd [("P",[(mname,msort)],rb)]) 
                            |> allI (x,valuest)
        val fvarP = mk_fvar "P" [(x,valuest),dest_var value_set] [input_set,value_set]
        val spec_P2funuex = 
            P2fun_uex
                |> inst_thm (mk_inst [(("A",set_sort),powt),
                                      (("B",set_sort),powt)] []) 
                |> fVar_prpl_th (mk_fd [("P",[(x,valuest),dest_var value_set]  ,incond)])
