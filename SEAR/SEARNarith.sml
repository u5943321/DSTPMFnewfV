...
val Nrec_def = P2fun_uex' |> qtinst_thm [(‘A’,‘N’),(‘B’,‘X’)]
                      |> qfVar_prpl_th1  
("P",[‘n:mem(N)’,‘x:mem(X)’],‘IN(Pair(n,x),Ninds(f0:X->X,x0))’)                    |> C mp (Nind_uex |> spec_all
                                        |> qgen ‘n’)
...
(rpt strip_tac >>
 ...
 assume_tac $ EQ_fvar "P" [("a",mem_sort (rastt "N"))] [assume “a0 = Suc(n)”] >>
 first_assum $ irule o iffRL >>
 last_x_assum irule  >> rw[Lt_Suc_Le] >> first_x_assum accept_tac)
(form_goal
 “(!a:mem(N).(!a0. Lt(a0,a) ==> P[n:mem(N)](a0)) ==> P[n:mem(N)](a)) ==> !a:mem(N). P[n:mem(N)](a)”));

val WOP = prove_store("WOP",
e0
(rpt strip_tac >> ccontra_tac >>
 by_tac “!l:mem(N). P[n:mem(N)](l) ==> ?n:mem(N). P[n:mem(N)](n) & ~(Le(l,n))”
 >-- (rpt strip_tac >> ccontra_tac >>
      suffices_tac “?a0:mem(N). P[n:mem(N)](a0) & !a1:mem(N). P[n:mem(N)](a1)==> Le(a0,a1)”
     >-- (rw[]>> first_x_assum accept_tac) >>
     qexists_tac ‘l’ >> strip_tac (* 2 *)
     >-- first_x_assum accept_tac >>
     rpt strip_tac >> ccontra_tac >>
     by_tac “?n:mem(N). P[n:mem(N)](n) & ~(Le(l,n))”
     >-- (qexists_tac ‘a1’ >> strip_tac >> first_x_assum accept_tac) >>
     first_x_assum opposite_tac ) >>
 suffices_tac “!n:mem(N). ~(P[n:mem(N)](n))” >-- (rpt strip_tac >>
 first_x_assum (qspecl_then [‘a’] assume_tac) >> 
 first_x_assum opposite_tac) >>
 suffices_tac “!n:mem(N) n0:mem(N). Le(n0,n) ==> ~P[n:mem(N)](n0)”
 >-- (strip_tac >> rpt strip_tac >> first_x_assum irule >>
     qexists_tac ‘n’ >> rw[Le_refl]) >>
 ind_with N_induct >> rpt strip_tac (* 2 *) >--
 (drule Le_O >>
 assume_tac $ EQ_fvar "P" [("n",mem_sort (rastt "N"))]  [assume “n0 = O”] >>
 ccontra_tac >> 
 ...
 ccontra_tac >>
 assume_tac $ EQ_fvar "P" [("n",mem_sort (rastt "N"))] [assume “n0 = Suc(n)”] >>
 first_x_assum $ drule o iffLR >> 
...
(form_goal
 “!a. P[n:mem(N)](a:mem(N)) ==> ?a0. P[n:mem(N)](a0) & !a1.P[n:mem(N)](a1) ==> Le(a0,a1)”));
...
val LE_def = 
AX1 |> qtinst_thm [(‘A’,‘N’),(‘B’,‘N’)]
|> qfVar_prpl_th1 ("P",[‘m:mem(N)’,‘n:mem(N)’],‘Sub(m,n) = O’)
|> qsimple_uex_spec "LE" []
|> store_as "LE_def";
...
val LT_def = 
AX1 |> qtinst_thm [(‘A’,‘N’),(‘B’,‘N’)]
|> qfVar_prpl_th1 ("P",[‘m:mem(N)’,‘n:mem(N)’],‘Holds(LE,m,n) & ~(m = n)’)
|> qsimple_uex_spec "LT" []
|> store_as "LT_def";
...
local
val l = WOP |> qfVar_prpl_th1 ("P",[‘a:mem(N)’],‘a = Suc(a)’)
in
...
(rw[GSYM IN_NONEMPTY] >> rpt strip_tac >>
 ...
 (WOP 
 |> qspecl [‘a:mem(N)’] 
 |> qfVar_prpl_th1 ("P",[‘a:mem(N)’],‘IN(a,s:mem(Pow(N)))’)) >>
 first_x_assum drule >> arw[])
(form_goal 
“!s:mem(Pow(N)).
   ~(s = Empty(N)) ==>
...
(rpt strip_tac >> x_choosel_then ["s"] strip_assume_tac
 ...
 |> qfVar_prpl_th1 ("P",[‘n:mem(N)’],‘?k. Add(n,Mul(k,d)) = a’)) >>
 qby_tac ‘~(s = Empty(N))’ 
 >-- (rw[GSYM IN_NONEMPTY] >> 
