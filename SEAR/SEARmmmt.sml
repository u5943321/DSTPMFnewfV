...
e0
(strip_tac >> irule 
 (P2fun_uex |> qtinst_thm ([(‘B’,‘Pow(W)’)])
|> qfVar_prpl_th1 ("P",[‘a:mem(A)’,‘s:mem(Pow(W))’],
‘!w.IN(w,s) <=> IN(a,App(Vof(M:mem(Pow(W * W) * Exp(W,Pow(A)))),w))’)) >>
 strip_tac >>
 accept_tac
 (IN_def_P |> qtinst_thm [(‘A’,‘W’)]
           |> qfVar_prpl_th1 ("P",[‘w:mem(W)’],
...
e0
(strip_tac >> irule 
 (P2fun_uex |> qtinst_thm [(‘A’,‘Pow(W)’),(‘B’,‘Pow(W)’)]
 |> qfVar_prpl_th1 
    ("P",[‘s0:mem(Pow(W))’,‘s:mem(Pow(W))’],‘!w.IN(w,s) <=> 
     ?w0. IN(w0,s0) & Rm(M:mem(Pow(W * W) * Exp(W,Pow(A))),w,w0)’)) >>
 strip_tac >>
 accept_tac
 (IN_def_P |> qtinst_thm [(‘A’,‘W’)]
 |> qfVar_prpl_th1 ("P",[‘w:mem(W)’],‘?w0. IN(w0,x) & Rm(M:mem(Pow(W * W) * Exp(W,Pow(A))),w,w0)’)))
(form_goal
...
(x_choose_then "s" (assume_tac o conjE1) 
 (IN_def_P_expand |> qtinst_thm [(‘A’,‘form(A)’)]) >> arw[] >>
 disch_tac >> 
 match_mp_tac PE_ind >> arw[])
(form_goal
 “P[f:mem(form(A))](Top(A)) & P[f:mem(form(A))](Bot(A)) & (!p:mem(A). P[f:mem(form(A))](Var(p))) &
      (!f1:mem(form(A)) f2. P[f:mem(form(A))](f1) & P[f:mem(form(A))](f2) ==> P[f:mem(form(A))](Conj(f1,f2))) & 
      (!f1:mem(form(A)) f2. P[f:mem(form(A))](f1) & P[f:mem(form(A))](f2) ==> P[f:mem(form(A))](Disj(f1,f2))) & 
      (!f:mem(form(A)).P[f:mem(form(A))](f) ==> P[f:mem(form(A))](Diam(f))) ==> 
      (!f:mem(form(A)). PE(f) ==> P[f:mem(form(A))](f)) ”));
...
(rpt strip_tac >>
 accept_tac
 (IN_def_P |> qtinst_thm [(‘A’,‘W’)]
|> qfVar_prpl_th1 ("P",[‘v:mem(W)’],‘Rm(M:mem(Pow(W * W) * Exp(W,Pow(A))),w,v)’)))
...
(form_goal “(?f:A->B. P[f:A->B](f)) <=> ?m:mem(Exp(A,B)). P[f:A->B](tof(m))”));
...
>-- (qexists_tac ‘tof(m)’ >> arw[Tpm_tof_inv]) >>
qexists_tac ‘Tpm(f)’ >> arw[tof_Tpm_inv])
(form_goal “(?m:mem(Exp(A,B)). P[m:mem(Exp(A,B))](m)) <=>
 (?f:A->B. P[m:mem(Exp(A,B))](Tpm(f)))”));
...
>-- (uex_tac >> 
    ...
    |> qfVar_prpl_th1 ("P",[‘g:A->B’],
‘P[f:A->B](g:A ->B) & !g'. P[f:A->B](g') ==> g' = g’)) >>
 ...
 pop_assum (assume_tac o uex_expand) >>
 assume_tac (fun_mem_ex_iff 
                 |> qfVar_prpl_th1 ("P",[‘g:A->B’],‘P[f:A->B](g:A ->B) & !g'. P[f:A->B](g') ==> g' = g’)) >>
...
(strip_tac >> assume_tac
 (fun_mem_uex_iff 
 |> qtinst_thm [(‘A’,‘UFs(W)’),(‘B’,‘Pow(A)’)]
 |> qfVar_prpl_th1 ("P",[‘f:UFs(W) -> Pow(A)’],‘!u a. IN(a,App(f:UFs(W)->Pow(A),u)) <=>
  IN(Tat(Vof(M),a),Repu(u))’)) >>
 first_x_assum (irule o iffLR) >>
 irule 
 (P2fun_uex |> qtinst_thm [(‘A’,‘UFs(W)’),(‘B’,‘Pow(A)’)]
 |> qfVar_prpl_th1 ("P",[‘x:mem(UFs(W))’,‘s:mem(Pow(A))’],
‘!a.IN(a,s) <=> IN(Tat(Vof(M:mem(Pow(W * W) * Exp(W,Pow(A)))),a),Repu(x))’)) >>
 strip_tac >> 
 accept_tac
 (IN_def_P 
 |> qfVar_prpl_th1 ("P",[‘a:mem(A)’],‘IN(Tat(Vof(M:mem(Pow(W * W) * Exp(W,Pow(A)))),a),Repu(x))’)))
(form_goal “!M:mem(Pow(W * W) * Exp(W,Pow(A))).
...
(rpt strip_tac >>
 accept_tac
 (IN_def_P |> qtinst_thm [(‘A’,‘W’)]
           |> qfVar_prpl_th1 ("P",[‘w:mem(W)’],‘?v.Rm(M:mem(Pow(W * W) * Exp(W,Pow(A))),w,v) & IN(v,X)’)))
...
(IN_def_P |> qtinst_thm [(‘A’,‘UFs(W) * UFs(W)’)]
|> qfVar_prpl_th1 ("P",[‘u12:mem(UFs(W) * UFs(W))’],
‘!X.IN(X,Repu(Snd(u12))) ==> 
...
(rw[GSYM IN_EXT_iff,csee_def,IN_Compl,osee_def] >>
 rpt strip_tac >>
 assume_tac
 (forall_exists_dual |> qtinst_thm [(‘A’,‘W’)]
 |>  qfVar_prpl_th1 ("P",[‘v:mem(W)’],
‘Rm(M:mem(Pow(W * W) * Exp(W,Pow(A))),x,v) ==> IN(v,X)’)) >>
 arw[] >> rw[neg_imp_conj])
(form_goal
...
(rpt strip_tac >>
 ...
 |> qfVar_prpl_th1 ("P",[‘s:mem(Pow(A))’],‘IN(a:mem(A),s)’)) >>
 arw[])
(form_goal “!A a:mem(A).
 ?!ss. !s. IN(s,ss) <=> IN(a,s)”)
...
e0
(strip_tac >> irule
 (P2fun_uex |> qtinst_thm [(‘A’,‘form(A)’),(‘B’,‘Pow(W)’)]
            |> qfVar_prpl_th1 ("P",[‘f:mem(form(A))’,‘s:mem(Pow(W))’],‘!w. IN(w,s) <=> satis(M:mem(Pow(W * W) * Exp(W,Pow(A))),w,f)’)) >>
 strip_tac >> accept_tac
 (IN_def_P |> qtinst_thm [(‘A’,‘W’)]
           |> qfVar_prpl_th1 ("P",[‘w:mem(W)’],‘satis(M:mem(Pow(W * W) * Exp(W,Pow(A))),w,x)’)))
(form_goal “!M:mem(Pow(W * W) * Exp(W,Pow(A))).
...
(strip_tac >> strip_tac >> 
 ...
 >-- accept_tac (IN_def_P_ex |> GSYM |> qtinst_thm [(‘A’,‘Pow(W)’)]
 |> qfVar_prpl_th1 ("P",[‘Y:mem(Pow(W))’],‘IN(osee(M:mem(Pow(W * W) * Exp(W,Pow(A))),Y),Repu(u))’))
 >> pop_assum strip_assume_tac >>
...
e0
(ind_with (Fin_induct |> qtinst_thm [(‘X’,‘form(A)’)]) >>
...
‘?ss. Fin(ss) & SS(ss,fs) & 
 ...
 >-- accept_tac (IN_def_P_ex |> GSYM |> qtinst_thm [(‘A’,‘Pow(W)’)]
 |> qfVar_prpl_th1 ("P",[‘Y:mem(Pow(W))’],‘IN(osee(M:mem(Pow(W * W) * Exp(W,Pow(A))),Y),Repu(w))’))
 >> pop_assum strip_assume_tac >>
...
e0
(rpt strip_tac >>
 strip_assume_tac (AX1 |> qtinst_thm  [(‘A’,‘W1’),(‘B’,‘W2’)] 
 |> qfVar_prpl_th1 ("P",[‘v1:mem(W1)’,‘v2:mem(W2)’],
‘!phi:mem(form(A)). PE(phi) ==> 
 ...
 (accept_tac (IN_def_P_ex |> GSYM |> qtinst_thm [(‘A’,‘form(A)’)] 
 |> qfVar_prpl_th1 ("P",[‘phi:mem(form(A))’],
...
(rpt strip_tac >> ccontra_tac >>
 assume_tac
 (exists_forall_dual |> qtinst_thm [(‘A’,‘Pow(form(A))’)]
 |> qfVar_prpl_th1 ("P",[‘ffs:mem(Pow(form(A)))’],‘SS(ffs:mem(Pow(form(A))), fs) & Fin(ffs) & ENT(ffs, phi)’)) >>
 fs[] >>
 pop_assum (K all_tac) >>
 qsuff_tac 
 ...
 fs[MCOMPACT_def] >> first_x_assum irule >> 
 rpt strip_tac >>
...
(form_goal
 “MCOMPACT ==> !fs:mem(Pow(form(A))) phi.ENT(fs,phi) ==>
...
(rpt strip_tac >>
 accept_tac
 (IN_def_P |> qtinst_thm [(‘A’,‘form(A)’)] 
 |> qfVar_prpl_th1 ("P",[‘phi:mem(form(A))’],‘PE(phi) & Ent(f,phi:mem(form(A)))’)))
...
e0
(strip_tac >> ind_with (Fin_induct |> qtinst_thm [(‘X’,‘B’)]) >>
...
(rpt strip_tac >> 
 ...
 >-- accept_tac (IN_def_P |> qtinst_thm [(‘A’,‘form(A)’)]
              |> qfVar_prpl_th1 
("P",[‘psi:mem(form(A))’],‘?psi0:mem(form(A)). psi = Neg(psi0) & PE(psi0) & 
 ...
 >-- (rpt strip_tac >> ccontra_tac >>
     ...
     >-- accept_tac (IN_def_P |> qtinst_thm [(‘A’,‘form(A)’)]
              |> qfVar_prpl_th1
 ("P",[‘f0:mem(form(A))’],‘IN(Neg(f0),Del(ss,f:mem(form(A))))’)            |> uex2ex_rule) >> 
     pop_assum strip_assume_tac >>
     qby_tac ‘Fin(ss0)’ >-- 
     (irule Fin_Inj >>
     ...
     >-- (rpt strip_tac >> 
         ...
         fs[Neg_eq_eq]) >>
     first_x_assum drule >> fs[])>>
 fs[MCOMPACT_def] >> first_x_assum drule >>  pop_assum strip_assume_tac >>
 ...
 >-- (drule $ iffLR PUS_def >> first_x_assum drule >>
     first_x_assum drule >> first_x_assum drule >> arw[]) >>
 qsspecl_then [‘M’,‘w’] assume_tac Prop_5_7 >> fs[MEQ_def])
(form_goal “MCOMPACT ==> !A f:mem(form(A)). PUS(f) ==> ?f0. PE(f0) & EQV(f,f0)”))
