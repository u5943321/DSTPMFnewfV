...
local
val lemma = 
    AX1 |> inst_thm (qmk_tinst [(‘B’,‘A’)])
        |> qfVar_prpl_th
        [("P",[‘a:mem(A)’,‘b:mem(A)’],‘~(a:mem(A) = a)’)] 
...
(rpt strip_tac >>
 strip_assume_tac
 (AX1_ex |> inst_thm (qmk_tinst [(‘A’,‘E1’),(‘B’,‘E2’)])
         |> qfVar_prpl_th [("P",[‘a:mem(E1)’,‘b:mem(E2)’], ‘F’)]) >>
 strip_assume_tac
 (AX1_ex |> inst_thm (qmk_tinst [(‘A’,‘E2’),(‘B’,‘E1’)])
         |> qfVar_prpl_th [("P",[‘a:mem(E2)’,‘b:mem(E1)’], ‘F’)]) >>
 qby_tac ‘isFun(R)’
...
val qtinst_thm = inst_thm o qmk_tinst 

local 
val lemma = 
    AX1 |> qtinst_thm [(‘B’,‘A’)]
        |> qfVar_prpl_th [("P",[‘y:mem(A)’,‘z:mem(A)’],‘y = a0:mem(A) & z = a0’)]
val lemma' = dimp_mp_l2r lemma (uex_def $ concl lemma)
in
...
val lemma = 
    AX1 |> qtinst_thm [(‘B’,‘1’)] 
        |> qfVar_prpl_th [("P",[‘a:mem(A)’,‘b:mem(1)’],‘a = a:mem(A)’)] 
        |> uex_expand
in
...
local
val l0 = 
    AX1 |> qtinst_thm [(‘A’,‘1’),(‘B’,‘A’)] 
        |> qfVar_prpl_th 
[("P",[‘a:mem(1)’,‘b:mem(A)’],‘a = a:mem(1) & P[x:mem(A)](b:mem(A))’)]
val uth = uex_def “?!R:1~>A. !a. Holds(R, dot, a) <=> P[x:mem(A)](a)”
in
...
(assume_tac l0 >> 
 ...
 rpt strip_tac >> rw[])
(form_goal
“?!R:1~>A.!a:mem(A). Holds(R,dot,a) <=> P[x:mem(A)](a)”)
end

(*
basic_fconv no_conv no_fconv “P[x:mem(A)](a) <=> P[a:mem(A)](a)”
taut_fconv 

rewr_fconv (eqT_intro (frefl (mk_fvar "f0" [] []))) “P[x:mem(A)](a) <=> P[a:mem(A)](a)”
*)
...
local 
val lemma = 
    AX1 |> qfVar_prpl_th [("P",[‘a:mem(A)’,‘b:mem(B)’],‘T’)] in
...
(rpt gen_tac >> strip_tac (* 2 *)
 >-- fs[isPair_def] >>
     ...
     drule
     (P2fun |> qtinst_thm [(‘A’,‘AB’),(‘B’,‘AB'’)]
            |> qfVar_prpl_th [("P",[‘ab:mem(AB)’,‘ab':mem(AB')’],‘App(p1:AB->A,ab) = App(p1':AB'->A,ab') & 
...
(rpt strip_tac >> rw[SetPr_def] >> rpt strip_tac >>
 ...
 strip_assume_tac $
 uex_expand $ 
 (AX1 |> qtinst_thm [(‘A’,‘X’),(‘B’,‘A * B’)]
 |> qfVar_prpl_th [("P",[‘x:mem(X)’,‘ab:mem(A * B)’],‘App(p1(A,B),ab) = App(f:X->A,x) & 
...
local
val lemma =
    Thm_2_4 |> qfVar_prpl_th [("P",[‘a:mem(A)’],‘App(f:A->X,a) = App(g,a)’)]
            |> gen_all 
val lemma1 = 
    AX1 |> qtinst_thm [(‘A’,‘X’),(‘B’,‘E’)] 
        |> qfVar_prpl_th [("P",[‘a0:mem(X)’,‘a0':mem(E)’],‘App(e:E->A,a0') = App(x:X->A,a0)’)]
        |> uex_expand
in
...
fun qfVar_prpl_th1 fv = qfVar_prpl_th [fv]

local
val lemma =
    Thm_2_4 |> qtinst_thm [(‘A’,‘B’)] 
            |> qfVar_prpl_th1 ("P",[‘b:mem(B)’],‘?a:mem(A).App(f:A->B,a) = b’) 
val lemma1 = 
    AX1 |> qtinst_thm [(‘B’,‘s’)] 
        |> qfVar_prpl_th1 ("P",[‘x:mem(A)’,‘y:mem(s)’],‘App(f:A->B,x) = App(m:s->B,y)’)
        |> uex_expand
in
...
(rpt strip_tac >>
 ...
 >-- (strip_tac >>
     strip_assume_tac
     (AX1_ex |> qtinst_thm [(‘A’,‘1’),(‘B’,‘A’)] 
         |> qfVar_prpl_th1 ("P",[‘a:mem(1)’,‘b:mem(A)’],
‘Holds(e:A~>PA,b,s)’))  >>
     first_x_assum (qspecl_then [‘R’] assume_tac) >>
     first_x_assum (qspecl_then [‘dot’] assume_tac) >> 
     fs[]) >>
 drule 
 (P2fun |> qtinst_thm [(‘A’,‘PA’),(‘B’,‘PA'’)] 
        |>  qfVar_prpl_th1 ("P",[‘s:mem(PA)’,‘s':mem(PA')’],
...
‘Holds(e':A~>PA',b,s')’))  >>
 ...
 drule 
 (P2fun |> qtinst_thm [(‘A’,‘PA'’),(‘B’,‘PA’)]
        |> qfVar_prpl_th1 ("P",[‘s':mem(PA')’,‘s:mem(PA)’],‘∀x. Holds(e:A~>PA,x,s) <=> Holds(e':A~>PA',x,s')’)
           ) >>
 pop_assum strip_assume_tac >>
 qexistsl_tac [‘f’,‘f'’] >>  
...
(*base change*)
val BC0_def = 
    AX1 |> qtinst_thm [(‘A’,‘Pow(Y)’),(‘B’,‘Pow(Z)’)] 
        |> qfVar_prpl_th1 ("P",[‘ys:mem(Pow(Y))’,‘zs:mem(Pow(Z))’],‘!z:mem(Z). Holds(In(Z),z,zs) <=> Holds(In(Y),App(f,z),ys)’)
...
val lemma = 
AX1 |> qtinst_thm [(‘A’,‘1’),(‘B’,‘A’)]
|> qfVar_prpl_th1 ("P",[‘star:mem(1)’,‘a:mem(A)’],
‘(?a0. a = App(s0:A0->A,a0))’)
|> uex_expand 
in
...
local
val lemma = 
Thm_2_4 |> qfVar_prpl_th1 ("P",[‘a:mem(A)’],‘(?a0. a = App(s0:A0->A,a0))’)
in
...
(uex_tac >> 
 ...
 pop_assum (assume_tac o GSYM) >> arw[])
(form_goal
 “?!s:mem(Pow(A)).!a.P[a:mem(A)](a) <=> Holds(In(A),a,s)”));
end
...
local
val lemma = 
In_def_P |> qfVar_prpl_th1 ("P",[‘a:mem(A)’],‘Holds(In(A),a,s1)’) |> uex_expand
in
...
local
val lemma = 
In_def_P |> qtinst_thm [(‘A’,‘Z’)]
|> qfVar_prpl_th1 ("P",[‘z:mem(Z)’],‘Holds(In(Y), App(f:Z->Y, z), a)’) 
|> uex_expand
in
...
val Ex0_def = 
AX1 |> qtinst_thm [(‘A’,‘Pow(Z)’),(‘B’,‘Pow(Y)’)]
    |> qfVar_prpl_th1 ("P",[‘zs:mem(Pow(Z))’,‘ys:mem(Pow(Y))’],‘!y:mem(Y). Holds(In(Y),y,ys) <=> ?z:mem(Z).Holds(In(Z),z,zs) & App(f,z) = y’)
    |> qsimple_uex_spec "Ex0" [‘f’] |> gen_all


val All0_def = 
AX1 |> qtinst_thm [(‘A’,‘Pow(Z)’),(‘B’,‘Pow(Y)’)]
    |> qfVar_prpl_th1 ("P",[‘zs:mem(Pow(Z))’,‘ys:mem(Pow(Y))’],‘!y:mem(Y). Holds(In(Y),y,ys) <=> !z:mem(Z). App(f,z) = y ==> Holds(In(Z),z,zs)’)
...
local
val lemma = 
In_def_P |> qtinst_thm [(‘A’,‘Y’)]
         |> qfVar_prpl_th1 ("P",[‘y:mem(Y)’],‘?z:mem(Z).Holds(In(Z),z,a) & App(f:Z->Y,z) = y’)
         |> uex_expand 
in
...
val lemma = 
In_def_P |> qtinst_thm [(‘A’,‘Y’)] 
         |> qfVar_prpl_th1 ("P",[‘y:mem(Y)’],‘!z:mem(Z). App(f:Z->Y,z) = y ==> Holds(In(Z),z,a)’)
         |> uex_expand 
in
...
val Join_def = 
    AX1 |> qfVar_prpl_th1 ("P",[‘a:mem(A)’,‘b:mem(B)’],
                           ‘Holds(R1:A~>B,a,b) | Holds(R2:A~>B,a,b)’)|> qsimple_uex_spec "Join" [‘R1’,‘R2’]
        |> gen_all
...
(rpt strip_tac >> rw[Sub_def,Meet_def,op_def,GSYM ao_def] >>
 rpt strip_tac >> qexists_tac ‘b1’ >> arw[] >>
 qexists_tac ‘b’ >> arw[])
(form_goal
...
(rpt strip_tac >> rw[GSYM ao_def,Join_def,GSYM R_EXT] >>
 ...
 >-- (disj1_tac >> qexists_tac ‘b1’ >> arw[])
 >-- (disj2_tac >> qexists_tac ‘b1’ >> arw[])
 >-- (qexists_tac ‘b1’ >> rpt strip_tac (* 2 *)
      >-- (disj1_tac >> arw[]) >> arw[]) >>
 qexists_tac ‘b1’ >> rpt strip_tac (* 2 *)
  >-- (disj2_tac >> arw[]) >> arw[])
(form_goal
...
local 
val lemma = 
AX1 |> qtinst_thm [(‘A’,‘B’),(‘B’,‘C’)]
    |> qfVar_prpl_th1 ("P",[‘b:mem(B)’,‘c:mem(C)’],‘
...
(rpt strip_tac >> rw[Sub_def,GSYM ao_def] >>
 ...
 (rpt strip_tac >> first_x_assum $ irule o iffLR >>
  qexists_tac ‘b1’ >> arw[] >> first_x_assum irule >> arw[]) >>
...
local
val lemma = 
AX1 |> qtinst_thm [(‘A’,‘B’),(‘B’,‘Pow(A)’)]
    |> qfVar_prpl_th1 ("P",[‘y:mem(B)’,‘s:mem(Pow(A))’],
‘!x.Holds(In(A),x,s) <=> Holds(R:B~>A,y,x)’) 
|> uex_expand
val lemma1 = 
In_def_P |> qfVar_prpl_th1 ("P",[‘x:mem(A)’],‘Holds(R:B~>A,a,x)’) |> uex_expand
in
...
val lemma =
AX1 |> qtinst_thm [(‘A’,‘1’),(‘B’,‘A’)]
    |> qfVar_prpl_th1 ("P",[‘star :mem(1)’,‘x:mem(A)’],
‘x = a:mem(A)’) |> uex_expand 
in
...
local
val l = P2fun_uex
               |> qfVar_prpl_th1 ("P",[‘a:mem(A)’,‘b:mem(B)’],‘App(f:A * X->B,Pair(a,x)) = b’) 
in
...
(rpt strip_tac >> 
 ...
 >-- (strip_tac >>
     ...
     flip_tac >> arw[]) >>
 drule 
 (P2fun |> qtinst_thm [(‘A’,‘A2B’),(‘B’,‘A2B'’)]
        |> qfVar_prpl_th1 ("P",[‘a2b:mem(A2B)’,‘a2b':mem(A2B')’],‘∀a. App(Ap1(ev:A * A2B -> B,a2b),a) = 
...
local 
val lemma = 
Thm_2_4 |> qtinst_thm [(‘A’,‘Pow(A * B)’)] 
        |> qfVar_prpl_th1 ("P",[‘s:mem(Pow (A * B))’],
‘!x:mem(A).?!y:mem(B).Holds(In(A * B),Pair(x,y),s)’)
val lemma1 = 
AX1 |> qtinst_thm [(‘A’,‘A2B * A’)]
    |> qfVar_prpl_th1 ("P",[‘fa: mem(A2B * A)’,‘b:mem(B)’],
‘Holds(In(A * B),Pair(App(p2(A2B,A),fa),b),App(i,App(p1(A2B,A),fa)))’) |> uex_expand
val lemma2 = 
In_def_P |> qtinst_thm [(‘A’,‘A * B’)]
         |> qfVar_prpl_th1 ("P",[‘ab:mem(A * B)’],
...
local
val l = 
AX1 |> qtinst_thm [(‘A’,‘A * A2B’)]
    |> qfVar_prpl_th1 ("P",[‘af:mem(A * A2B)’,‘b:mem(B)’],
‘App(ev:A2B * A ->B,Pair(App(p2(A,A2B),af),App(p1(A,A2B),af))) = b’) |> uex_expand  
in
...
local 
val l = P2fun' |> qtinst_thm [(‘A’,‘X’),(‘B’,‘Exp(A,B)’)]
|> qfVar_prpl_th1 ("P",[‘x:mem(X)’,‘fx:mem(Exp(A,B))’],
...
e0
(strip_assume_tac IN_def_P_expand >>
 qexists_tac ‘s’ >> first_x_assum accept_tac)
(form_goal “?s:mem(Pow(A)). (!a. P[a:mem(A)](a) <=> IN(a,s))”));
...
local
val lemma =
AX1 |> qtinst_thm [(‘A’,‘Pow(Pow(A))’),(‘B’,‘Pow(A)’)] 
|> qfVar_prpl_th1 ("P",[‘sss:mem(Pow(Pow(A)))’,‘ss:mem(Pow(A))’],‘!a:mem(A).IN(a,ss) <=> !ss0. IN(ss0,sss) ==> IN(a,ss0)’)
|> uex_expand 
val lemma' = 
IN_def_P_expand |> qfVar_prpl_th1 ("P",[‘a:mem(A)’],
‘!ss0. IN(ss0,sss) ==> IN(a:mem(A),ss0)’) |> GSYM 
in
...
(rpt strip_tac >>
 ...
 drule (P2fun |> qtinst_thm [(‘A’,‘B’),(‘B’,‘B'’)]
              |> qfVar_prpl_th1 ("P",[‘b:mem(B)’,‘b':mem(B')’],‘App(i:B->A,b) = App(i':B'->A,b')’)) >>
 pop_assum strip_assume_tac >>
 qby_tac
 ‘∀b'. ?!b. App(i,b) = App(i',b')’ 
...
(form_goal “
 ...
 (Inj(i') & 
      (∀a. P[a:mem(A)](a:mem(A)) ⇔ ∃b:mem(B'). a = App(i',b))) ⇒
  ∃f:B -> B' g:B' -> B. 
     f o g = Id(B') &
     g o f = Id(B) &
     i' o f = i & i o g = i'”)
...
val N_def = Thm_2_4' |> qtinst_thm [(‘A’,‘N0’)]
                     |> qfVar_prpl_th1 
...
(rpt strip_tac >> 
 ...
 >-- (strip_tac >> uex_tac >> qexists_tac ‘im’ >> arw[] >>
     ...
     irule IN_EXT >> arw[]) >>
irule 
 (P2fun' |> qtinst_thm [(‘A’,‘Pow(A)’),(‘B’,‘Pow(B)’)]
         |> qfVar_prpl_th1 ("P",[‘x:mem(Pow(A))’,‘y:mem(Pow(B))’],‘!b. IN(b,y) <=> ?a. IN(a,x) & b = App(f:A->B,a)’)) >>
 strip_tac >> accept_tac
 (IN_def_P |> qtinst_thm [(‘A’,‘B’)] 
           |> qfVar_prpl_th1 ("P",[‘b:mem(B)’],‘?a:mem(A). IN(a,x) & b = App(f:A->B,a)’)))
...
val Whole_def = 
    IN_def_P |> GSYM |> qfVar_prpl_th1 ("P",[‘a:mem(A)’],‘T’)                |> rewr_rule [] 
...
(IN_def_P_expand |> qtinst_thm [(‘A’,‘N’)]) >>
 arw[] >> 
 ...
 >-- (pop_assum mp_tac >> pop_assum (K all_tac) >>
     ...
     qexists_tac ‘a'’ >> arw[]) >>
 qexists_tac ‘n’>> rw[])
(form_goal “P[n:mem(N)](O) & 
  (!n.P[n:mem(N)](n) ==> P[n:mem(N)](App(SUC,n)))==> !n:mem(N).P[n:mem(N)](n)”));
...
val Sg_def = P2fun_uex |> qtinst_thm [(‘B’,‘Pow(A)’)]
                   |> qfVar_prpl_th1 ("P",[‘a:mem(A)’,‘s:mem(Pow(A))’],‘!a0. IN(a0,s) <=> a0 = a:mem(A)’)
                   |> C mp 
                      (IN_def_P |> qfVar_prpl_th1 
("P",[‘a0:mem(A)’],‘a0 = a:mem(A)’)|> qgen ‘a’)
...
val Empty_def = IN_def_P |> qtinst_thm [(‘A’,‘X’)]
                         |> qfVar_prpl_th1 
                         ("P",[‘x:mem(X)’],‘F’)
...
(rpt strip_tac >> flip_tac >>
 ...
 irule (P2fun' 
            |> qfVar_prpl_th1 ("P",[‘x:mem(A)’,‘y:mem(B)’],‘y = App(Ev(A,B),Pair(x,f0:mem(Exp(A,B))))’)
) >>
...
(rpt strip_tac >>
 ...
 (Thm_2_4 |> qtinst_thm [(‘A’,‘Pow(A) * Pow(B)’)]
 |> qfVar_prpl_th1 
("P",[‘sab:mem(Pow(A) * Pow(B))’],
‘(Snd(sab) = Empty(B) & ?a. Fst(sab) = Sing(a)) | 
 ...
 >-- (irule (P2fun' |> qtinst_thm [(‘B’,‘Pow(A) * Pow(B)’)]
     |> qfVar_prpl_th1 ("P",[‘a:mem(A)’,‘sab:mem(Pow(A) * Pow(B))’],‘sab:mem(Pow(A) * Pow(B)) = Pair(Sing(a),Empty(B))’)) >>
     strip_tac >> uex_tac >> 
 ...
 >-- (irule 
          (P2fun'
               |> qtinst_thm [(‘A’,‘B’),(‘B’,‘Pow(A) * Pow(B)’)]
     |> qfVar_prpl_th1 ("P",[‘b:mem(B)’,‘sab:mem(Pow(A) * Pow(B))’],‘sab:mem(Pow(A) * Pow(B)) = Pair(Empty(A),Sing(b))’)) >>
     strip_tac >> uex_tac >> 
...
qby_tac ‘(!a b. ~(App(i1,a) = App(i2,b)))’
 ...
 (rw[iscoPr_def] >> rpt strip_tac >>
  ...
  ‘?fg: AB ->X.
   ...
   (!b. App(i,ab) = Pair(Empty(A),Sing(b)) ==>
        App(fg,ab) = App(g,b)) ’
  >-- (assume_tac (P2fun' |> qtinst_thm [(‘A’,‘AB’),(‘B’,‘X’)]
     |> qfVar_prpl_th1 ("P",[‘ab:mem(AB)’,‘x:mem(X)’],‘(?a. ab = App(i1:A->AB,a) & x = App(f:A->X,a)) | 
...
val i2_Inj = i2_def |> spec_all |> conjE2
                    |> conjE2 |> conjE1 |> gen_all
