...
open HolKernel Parse boolLib bossLib;
open HolKernel Parse boolLib bossLib;
open finite_setTheory tmTheory fm1Theory fVinst1Theory wfgenl1Theory wffTheory newdefsTheory PfDrvTheory listTheory;
...
>- (gs[fcong_def,wff_IFF] >>
    ...
    ‘(fprpl
             (mk_bmap (REVERSE (Lofeqthl (map2list (LENGTH sl − 1) eqths))))
             b) =
     fVinst (TO_FMAP [("",sl),b]) (fVar "" sl (Lofeqthl (map2list (LENGTH sl − 1) eqths)))’
     by (simp[fVinst_def,FDOM_TO_FMAP,TO_FMAP_SING]) >>
    ‘wfabsap Σf sl (Lofeqthl (map2list (LENGTH sl − 1) eqths))’ by (irule wfabsap_of_wfsl >>
      simp[LENGTH_LRofeqthl,LENGTH_map2list] >>
      ‘LENGTH sl ≠ 0’ by simp[] >> simp[] >>
      strip_tac >> strip_tac >>
      drule_then assume_tac EL_Lofeqthl >>
      simp[] >>
      first_x_assum $ drule_then strip_assume_tac >>
      gs[is_EQ_def,dest_eq_EQ,Leq_Req_EQ] >>
      Cases_on ‘eqths n’ >> Cases_on ‘r’ >>
      gs[concl_def]>>
      first_x_assum $ drule_then assume_tac >>
      gs[wff_EQ]) >>
    ‘wffVmap (Σf,Σp,Σe) (TO_FMAP [(("",sl),b)])’
     by simp[wffVmap_def,FDOM_TO_FMAP,TO_FMAP_SING] >>
    simp[] >> irule wff_fVinst >>
    gs[wfsig_def] >>
    irule wff_fVar' >> simp[wffstl_def] >>
    gs[wffVsl_def] >> metis_tac[]) >>
    drule_then assume_tac wfs_wffVsl >>
    ‘(fprpl
             (mk_bmap (REVERSE (Rofeqthl (map2list (LENGTH sl − 1) eqths))))
             b) =
     fVinst (TO_FMAP [("",sl),b]) (fVar "" sl (Rofeqthl (map2list (LENGTH sl − 1) eqths)))’
     by (simp[fVinst_def,FDOM_TO_FMAP,TO_FMAP_SING]) >>
    ‘wfabsap Σf sl (Rofeqthl (map2list (LENGTH sl − 1) eqths))’ by (irule wfabsap_of_wfsl >>
      simp[LENGTH_LRofeqthl,LENGTH_map2list] >>
      ‘LENGTH sl ≠ 0’ by simp[] >> simp[] >>
      strip_tac >> strip_tac >>
      drule_then assume_tac EL_Rofeqthl >>
      simp[] >>
      first_x_assum $ drule_then strip_assume_tac >>
      gs[is_EQ_def,dest_eq_EQ,Leq_Req_EQ] >>
      Cases_on ‘eqths n’ >> Cases_on ‘r’ >>
    ...
    gs[wfsig_def] >>
    irule wff_fVar' >> simp[wffstl_def] >>
    gs[wffVsl_def] >> metis_tac[] (* need to realise as fVinst *))
>~ [‘(Γ,A,f) = fcong (map2list (LENGTH sl − 1) eqths) sl b’]
   >- (gs[fcong_def,IN_Uof,MEM_map2list] >>
    ‘LENGTH sl ≠ 0’ by simp[] >>
    ‘n0 < LENGTH sl’ by simp[] >>
