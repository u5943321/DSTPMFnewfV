...
open finite_setTheory tmTheory fm1Theory fVinst1Theory listTheory;
open finite_mapTheory;
open pred_setTheory;

val _ = new_theory "wfgenl1";
...
Theorem fabs_False:
fabs v i False = False
Proof
rw[fabs_def]
QED        
        

Theorem mk_FALLL_False:
(mk_FALLL t False) = FALLL (vl2sl t) False
Proof
Induct_on ‘LENGTH t’
...
qspecl_then [‘LENGTH t'’,‘q’,‘r’,‘vl2sl t'’,‘False’,‘0’]
assume_tac mk_FALL_FALLL0 >>
gs[LENGTH_vl2sl, fabs_False]
QED
...
Theorem wfdpvl_False_fVar:
wfdpvl vl False ∧ ALL_DISTINCT vl ∧ okvnames vl ⇒
...
gs[mk_FALLL_False,ffv_FALLL,PULL_EXISTS,ffv_False] >>
...
wffstl Σf sl tl ⇔
wfabsap Σf sl tl ∧
∃vl. wfvl Σf vl False ∧ vl2sl vl = sl ∧ ALL_DISTINCT vl ∧
     okvnames vl
End          
...
fVslfv False = {}
Proof
simp[fVslfv_def,Uof_def,fVars_False]
QED

             
Theorem wfdpvl_False:
(wfdpvl [] False ⇔ T) ∧
(wfdpvl (h :: t) False ⇔
 wfdpvl t False ∧
 ∀n s. (n,s) ∈ slfv (vl2sl t) ⇒ h ∉ sfv s)
Proof
 simp[wfdpvl_def,fVslfv_False] >>
 rw[EQ_IMP_THM] (* 2 *)
 >> (drule_then assume_tac wfdpvl_ffv_mk_FALLL >>
    gs[mk_FALLL_False,ffv_FALLL,ffv_False,slfv_alt] >>
    metis_tac[])
QED    
...
∀nl. LENGTH nl = LENGTH sl ∧ ALL_DISTINCT nl ∧
 (set nl) ∩ (slnames sl) = {} 
⇒ wfdpvl (sl2vl nl sl) False
Proof
Induct_on ‘LENGTH sl’ >> gs[wfdpvl_False,sl2vl_def] >>
Cases_on ‘sl’ >> simp[] >> rw[] >>
Cases_on ‘nl’ >> gs[] >>
simp[sl2vl_def] >>
simp[wfdpvl_False] >>
‘wfdpvl (sl2vl t' (specsl 0 (Var h' h) t)) False’
 by (first_x_assum irule >> simp[LENGTH_specsl] >>
     gs[slnames_CONS] >>
     metis_tac[ALL_DISTINCT_EMPTY_INTER_lemma]) >>
rw[] >>
...
∀n s. (n,s) ∈ slfv (vl2sl t) ⇒ h ∉ sfv s
Proof
gs[wfdpvl_False] >> metis_tac[]
QED
        
Theorem wfdpvl_MEM_NOT_slfv:
∀n s. MEM (n,s) vl ∧ wfdpvl vl False ∧
      okvnames vl ⇒
      (n,s) ∉ slfv (vl2sl vl)
Proof
...
first_x_assum $ drule_then assume_tac >>
pop_assum mp_tac >> simp[] >> first_x_assum irule>>
simp[] >> gs[okvnames_CONS,wfdpvl_False]
QED
...
dep_rewrite.DEP_REWRITE_TAC[abssl_id] >> gs[IN_slfv] >>
rw[] (* 2 *) >- metis_tac[] >>
first_x_assum $ qspecl_then [‘t’] assume_tac >> gs[wfdpvl_False,okvnames_CONS] >>
gs[EXTENSION,EQ_IMP_THM] >> rw[] >> TRY (metis_tac[tm_tree_WF]) >>
cheat
QED
*)
...
Induct_on ‘LENGTH vl’ >> rw[] >>
Cases_on ‘vl’ >> simp[] >> rw[] >>
gs[wfdpvl_False,tlnames_CONS] >>
rw[] >> Cases_on ‘h’ >> gs[] >>
‘wfdpvl (MAP (λ(n0,s0). (n0,srename (n,s) nn s0)) t) False’ by (first_x_assum irule >> simp[] >>
          gs[okvnames_CONS] >>
...
Theorem wfvl_sl2vl_vl2sl:
 (∀fsym.
           ...
           BIGUNION {tfv (Var n s) | MEM (n,s) (fsymin Σf fsym)}) ⇒
  ∀nl n s.
    wfvl Σf vl False ∧ LENGTH nl = LENGTH vl ∧
    ALL_DISTINCT nl ∧
    okvnames vl ∧
...
Theorem variant_NOT_SUBSET:
∀s1 i. (variant s1 i) ∈ s2 ⇒ ∃j. j ∈ s2 ∧ ¬fIN j s1
Proof
recInduct variant_ind >> rw[] >>
pop_assum mp_tac >> simp[Once variant_def] >> rw[] >>
metis_tac[]
QED
...
>- (Induct_on ‘n’ >> simp[] >> gs[] >>
   qexists ‘variant (fromSet (set nl)) "" :: nl’ >>
   simp[] >>
   strip_tac >>
   drule_then strip_assume_tac variant_NOT_SUBSET >>
   gs[IN_fromSet]) >>
Cases_on ‘n’ >> gs[] >>
...
Theorem tnames_FINITE:
(∀t. FINITE (tnames t)) ∧ (∀s. FINITE (snames s))
Proof
ho_match_mp_tac better_tm_induction >> simp[PULL_EXISTS,tnames_thm] >>
 ‘∀l. {tnames t | MEM t l} = IMAGE tnames (set l)’  suffices_by simp[] >>
 simp[EXTENSION]
QED

Theorem tlnames_FINITE:
∀tl. FINITE (tlnames tl)
Proof
simp[tlnames_alt,PULL_EXISTS,tnames_FINITE] >>
‘∀tl. {tnames t | MEM t tl} = IMAGE tnames (set tl)’  suffices_by simp[] >>
 simp[EXTENSION]
QED 


Theorem slnames_FINITE:
∀sl. FINITE (slnames sl)
Proof
simp[slnames_alt,PULL_EXISTS,tnames_FINITE] >>
‘∀sl. {snames s | MEM s sl} = IMAGE snames (set sl)’  suffices_by simp[] >>
 simp[EXTENSION]
QED
...
‘∃nl.ALL_DISTINCT nl ∧
     ...
     (set nl) ∩ IMAGE FST (FDOM σ) = {} ∧
     set nl ∩ tlnames (MAP Var' vl) = ∅’
 by (‘FINITE (slnames (MAP (sinst σ) sl) ∪ IMAGE FST (FDOM σ) ∪ tlnames (MAP Var' vl))’ by simp[tlnames_FINITE,slnames_FINITE] >>
     drule_then assume_tac nl_EX >>
     first_x_assum $ qspecl_then [‘LENGTH sl’] assume_tac >> gs[] >>
     qexists ‘nl’ >> simp[] >>
     gs[EXTENSION] >> metis_tac[]) >>
...
Theorem vl2sl_fix:
(∀n s. MEM (n,s) vl ⇒  wfs Σf s ∧ n ∉ slnames (MAP SND vl)) ⇒
vl2sl vl = MAP SND vl
Proof        
Induct_on ‘vl’ >> simp[vl2sl_EMPTY] >>
simp[vl2sl_CONS] >> rw[] >> gs[slnames_CONS]>>
‘vl2sl vl = MAP SND vl’ by metis_tac[] >>
simp[] >> Cases_on ‘h’ >> 
irule abssl_id >> gs[MEM_MAP] >>
gs[slnames_def,IN_Uof,PULL_EXISTS,MEM_MAP] >>
rw[] >>
first_x_assum $ qspecl_then [‘q’,‘r’] assume_tac >> gs[] >>
first_x_assum $ qspecl_then [‘y’] assume_tac >> gs[] >>
metis_tac[tfv_tnames]
QED
...
Theorem wfs_wffVsl_wfvl_lemma:
(∀n s. MEM (n,s) vl ⇒  wfs Σf s ∧ n ∉ slnames (MAP SND vl)) ⇒
wfvl Σf vl False
Proof
Induct_on ‘vl’ >> simp[wfvl_alt] >> gs[slnames_CONS] >> Cases_on ‘h’ >>
strip_tac >>
‘wfvl Σf vl False’ by metis_tac[] >> simp[] >>
simp[IN_fVslfv,fVars_def] >>
gs[wfvl_def] >> 
drule_then assume_tac wfdpvl_ffv_mk_FALLL >> gs[] >>
‘vl2sl vl = MAP SND vl’ by metis_tac[vl2sl_fix] >> simp[] >>
gs[slnames_def,IN_Uof,PULL_EXISTS,MEM_MAP,IN_slfv] >>
rw[] >>
first_x_assum $ qspecl_then [‘q’,‘r’] assume_tac >> gs[] >>
first_x_assum $ qspecl_then [‘y’] assume_tac >> gs[] >>
metis_tac[tfv_tnames,vsort_tfv_closed]
...
Theorem specsl_wfs_fix:
(∀s. MEM s sl ⇒ wfs Σf s) ⇒ specsl i t sl = sl
Proof     
rw[] >> irule LIST_EQ >> simp[LENGTH_specsl] >>
rw[] >>
drule_then assume_tac specsl_EL >> simp[] >>
irule $ cj 2 trpl_id >>
‘sbounds (EL x sl) = {}’ suffices_by metis_tac[MEMBER_NOT_EMPTY] >>
irule $ cj 2 wft_no_bound >>
metis_tac[MEM_EL]
QED
   
Theorem sl2vl_no_bound:
∀sl nl. LENGTH nl = LENGTH sl ∧ (∀s. MEM s sl ⇒ wfs Σf s) ⇒ sl2vl nl sl = ZIP (nl,sl)
Proof
Induct_on ‘sl’ >> simp[sl2vl_def] >> Cases_on ‘h’ >> Cases_on ‘nl’ >>
simp[] >>
rw[sl2vl_def] >>
first_x_assum $ qspecl_then [‘t’] assume_tac >> gs[] >>
‘(specsl 0 (Var h (St s l)) sl) = sl’
 by (irule specsl_wfs_fix >> metis_tac[]) >> simp[]
QED 
...
irule_at Any vl2sl_sl2vl >>
simp[] >>
irule_at Any ALL_DISTINCT_sl2vl >> simp[] >>
irule_at Any okvnames_sl2vl >> simp[] >>
irule_at Any wfs_wffVsl_wfvl_lemma >>
drule_all_then assume_tac sl2vl_no_bound >> rw[] (* 2 *)
>- (gs[MEM_ZIP] >> metis_tac[MEM_EL]) >>
gs[] >> gs[slnames_def,MAP_ZIP] >>
simp[IN_Uof] >> gs[MEM_ZIP] >>
gs[EXTENSION,IN_Uof] >> metis_tac[MEM_EL] 
QED



                     
Theorem wfabsap_of_wfsl:
∀tl sl. (∀s. MEM s sl ⇒ wfs Σf s) ∧ LENGTH sl = LENGTH tl ∧
(∀n. n < LENGTH sl ⇒ wft Σf (EL n tl) ∧ sort_of (EL n tl) = EL n sl) ⇒
wfabsap Σf sl tl
Proof
...
>- (first_x_assum $ qspecl_then [‘0’] assume_tac >> gs[]) >>
‘(specsl 0 h' t) = t’ by metis_tac[specsl_wfs_fix] >>
first_x_assum irule >> simp[LENGTH_specsl] >>
rw[] (* 2 *) >>
