...
(rpt strip_tac >>
 ...
 assume_tac
 (uex_unique |> qtinst_thm [(‘A’,‘2’),(‘B’,‘Aop’)] 
 |> qfVar_prpl_th1 ("P",[‘f':2->Aop’],
    ‘isopf(f:2->A, f':2->Aop)’)) >>
...
(rpt strip_tac >>
 assume_tac
 (CC5 |> qtinst_thm [(‘A’,‘Aop’)] 
 |> fVar_prpl_th(mk_fd [("R",List.map (dest_var o rastt)["f:2->Aop","g:2->B"],
    “∃f0:2->A. isopf(f0,f:2->Aop) & R[a1:2->A,a2:2->B](f0,g:2->B)”)])) >>
 suffices_tac 
 “?(cf : fun(Aop, B)).
 !(a : fun(2, Aop))  (b : fun(2, B)).
    (?(f0 : fun(2, A)). isopf(f0, a) & R[a1:2->A,a2:2->B](f0, b)) <=>
                 cf o a = b”
 >-- (strip_tac >>
     qexists_tac ‘cf’ >>
     pop_assum (assume_tac o GSYM) >> arw[] >>
 ...
 suffices_tac
 “?(g : fun(2, B)). 
  ?(f0 : fun(2, A)). isopf(f0, f:2->Aop) & R[a1:2->A,a2:2->B](f0, g)”
 >-- (strip_tac >> uex_tac >>
     ...
     assume_tac
     (uex_unique |> qtinst_thm [(‘A’,‘2’)] 
     |> fVar_prpl_th (mk_fd [("P",List.map (dest_var o rastt) ["g:2->B"],
        “R[a1:2->A,a2:2->B](f0':2->A, g:2->B)”)])) >>
     first_x_assum drule >> first_x_assum irule >>
     arw[]) >>
 pop_assum (strip_assume_tac o uex2ex_rule) >>
 first_x_assum (qsspecl_then [‘f'’] assume_tac) >>
...
(form_goal
 ...
 (∀f:2->A g:2->B. R[a1:2->A,a2:2->B](f,g) ⇒ 
  R[a1:2->A,a2:2->B](id(dom(f)),id(cod(g))) ∧
  R[a1:2->A,a2:2->B](id(cod(f)),id(dom(g)))) ∧
 (∀f:2->A g:2->A h: 2->B. cpsb(g,f) ⇒
  R[a1:2->A,a2:2->B](g @ f, h) ⇒
  ∀f1 g1. R[a1:2->A,a2:2->B](f,f1) ∧ R[a1:2->A,a2:2->B](g,g1) ⇒ h = f1 @ g1) ⇒
 ∀Aop. isop(A,Aop) ⇒ 
 ∃cf:Aop->B. 
 ∀a:2->A a':2->Aop.
  isopf(a,a') ⇒
 ∀b:2->B.
  R[a1:2->A,a2:2->B](a,b) ⇔ cf o a' = b”));
...
(qsuff_tac
 ...
 rpt strip_tac >> 
 assume_tac
 (Thm18 |> qtinst_thm [(‘B’,‘A2’)] 
 |> qfVar_prpl_th1 ("R",[‘f:2->A’,‘g:2->A2’],
    ‘isopf(f:2->A,g:2->A2)’)) >>
 qsuff_tac
 ‘!(Aop : cat).
               isop(A, Aop) ==>
