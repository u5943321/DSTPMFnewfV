...
val _ = new_pred "areIso" [("A",cat_sort),("B",cat_sort)]
...
(*PCC2 in the paper has a typo, confirmed by contacting the author*)
...
“
 (∀f:2->A. ∃!g:2->B. R[a1:2->A,a2:2->B](f,g)) ∧
 (∀f:2->A g:2->B. R[a1:2->A,a2:2->B](f,g) ⇒ 
  R[a1:2->A,a2:2->B](id(dom(f)),id(dom(g))) ∧ R[a1:2->A,a2:2->B](id(cod(f)),id(cod(g)))) ∧
 (∀f:2->A g:2->A h: 2->B. cpsb(g,f) ⇒
  R[a1:2->A,a2:2->B](g @ f, h) ⇒ ∀f1 g1. R[a1:2->A,a2:2->B](f,f1) ∧ R[a1:2->A,a2:2->B](g,g1) ⇒ h = g1 @ f1) ⇒
 ∃cf:A->B. ∀a:2->A b:2->B. R[a1:2->A,a2:2->B](a,b) ⇔ cf o a = b”)
...
local
val l = CC5 |> qtinst_thm [(‘A’,‘B’),(‘B’,‘A’)] 
            |> qfVar_prpl_th1 ("R",[‘b:2->B’,‘a:2->A’],
‘b:2->B = f o a:2->A’)
in
...
(qsuff_tac
 ...
 >-- (strip_tac >> qexists_tac ‘cf’ >>
     ...
     arw[]) >>
 match_mp_tac
 (CC5 |> qtinst_thm [(‘A’,‘Exp(2,2)’),(‘B’,‘3’)] 
     |> qfVar_prpl_th1 ("R",[‘f:2->Exp(2,2)’,‘g:2->3’],
‘(f = Tp(lr𝟚) & g = α) |
         (f = Tp(ul𝟚) & g = β) |
         (f = Tp(v𝟚) & g = γ) |
         (f = Tp(c𝟙) & g = id(β₂)) |
...
(qsuff_tac
 ...
 >-- (strip_tac >> qexists_tac ‘cf’ >>
     ...
     arw[]) >>
 match_mp_tac
 (CC5 |> qtinst_thm [(‘A’,‘3’),(‘B’,‘Exp(2,2)’)] 
     |> qfVar_prpl_th1 ("R",[‘f:2->3’,‘g:2->Exp(2,2)’],
‘(f = α & g = Tp(lr𝟚)) |
         (f = β & g = Tp(ul𝟚)) |
         (f = γ & g = Tp(v𝟚)) |
         (f = id(β₂) & g = Tp(c𝟙)) |
...
val l = 
CC5 |> qtinst_thm [(‘A’,‘B’),(‘B’,‘C’)]
    |> qfVar_prpl_th1 ("R",[‘b:2->B’,‘c:2->C’],
...
(rpt strip_tac >>
 ...
 match_mp_tac
 (CC5 |> qtinst_thm [(‘B’,‘0’)] 
 |> qfVar_prpl_th1 ("R",[‘f:2->A’,‘g:2->0’],‘F’)) >> 
...
val l = CC5 |> qtinst_thm [(‘A’,‘B’),(‘B’,‘C’)]
|> qfVar_prpl_th1 ("R",[‘b:2->B’,‘c:2->C’],‘(dom(b:2->B) = B0 ∧ ~(cod(b) = B0) ∧ c:2->C = h) |
 (~(dom(b) = B0) ∧ cod(b) = B0 ∧ c = k) |
 (dom(b) = B0 ∧ cod(b) = B0 ∧ c = h o 𝟘) |
 (~(dom(b) = B0) ∧ ~(cod(b) = B0) ∧ c = h o 𝟙)’)
...
(rpt strip_tac >> 
 ...
 pop_assum (assume_tac o GSYM) >> arw[]) >>
 match_mp_tac
 (CC5 |> qtinst_thm [(‘A’,‘A + B’),(‘B’,‘A + B’)] 
 |> qfVar_prpl_th1 ("R",[‘f:2->A+B’,‘g:2->A+B’],
...
(dimp_tac >> strip_tac (* 2 *)
 >-- (ccontra_tac >> fs[]) >>
 ccontra_tac >>
 by_tac “!f:A->B.~P[f:A->B](f)”
 >-- (strip_tac >> ccontra_tac >>
     suffices_tac “?f:A->B. P[f:A->B](f)” >-- arw[] >> 
     qexists_tac ‘f’ >> arw[]) >>
 fs[])
(form_goal “(?f:A->B.P[f:A->B](f)) <=>
 ~(!f:A->B.~P[f:A->B](f))”));
...
(rpt strip_tac >> qcases ‘(∀fa:1->A.F) | (∀fb:1->B.F)’ (*2*)
 ...
 fs[neg_or_distr] >>
 fs[forall_exists_dual |> qtinst_thm [(‘A’,‘1’),(‘B’,‘A’)]
 |> qfVar_prpl_th1 ("P",[‘fa:1->A’],‘F’)] >>
...
(rpt strip_tac >>
 ...
 match_mp_tac
 (CC5 |> qtinst_thm [(‘A’,‘X’),(‘B’,‘S’)] 
 |> qfVar_prpl_th1 ("R",[‘x:2->X’,‘s:2->S’],
...
val cf_lemma =  CC5 |> qtinst_thm [(‘B’,‘Cl’)] 
 |> qfVar_prpl_th1 ("R",[‘f:2->A’,‘g:2->Cl’],
...
(rpt strip_tac >>
 ...
 >-- (strip_tac >> uex_tac >>
     ...
     ‘(!s1:1->S. ~(dom(a) = i o s1)) <=> 
      ~(?s1:1->S. dom(a) = i o s1)’
     >-- accept_tac(forall_exists_dual |> qtinst_thm [(‘A’,‘1’),(‘B’,‘S’)] 
     |> qfVar_prpl_th1 ("P",[‘s1:1->S’],‘~(dom(a:2->A) = i o s1:1->S)’) 
     |> rewr_rule[]) >>
     qby_tac
     ‘(!s2:1->S. ~(cod(a) = i o s2)) <=> 
     ~(?s2:1->S. cod(a) = i o s2)’
     >-- accept_tac(forall_exists_dual |> qtinst_thm [(‘A’,‘1’),(‘B’,‘S’)] 
     |> qfVar_prpl_th1 ("P",[‘s2:1->S’], ‘~(cod(a:2->A) = i o s2:1->S)’) 
     |> rewr_rule[]) >> 
...
‘?c:A->Cl. 
 ...
 ‘(!s1:1->S. ~(dom(u o x) = i o s1)) <=> 
  ~(?s1:1->S. dom(u o x) = i o s1)’
 >-- accept_tac(forall_exists_dual |> qtinst_thm [(‘A’,‘1’),(‘B’,‘S’)] 
     |> qfVar_prpl_th1 ("P",[‘s1:1->S’], ‘~(dom(u:A'->A o x:2->A') = i o s1:1->S)’) 
     |> rewr_rule[]) >>
 qby_tac
 ‘(!s2:1->S. ~(cod(u o x) = i o s2)) <=> 
  ~(?s2:1->S. cod(u o x) = i o s2)’
 >-- accept_tac(forall_exists_dual |> qtinst_thm [(‘A’,‘1’),(‘B’,‘S’)]  
     |> qfVar_prpl_th1 ("P",[‘s2:1->S’],‘~(cod(u:A'->A o x:2->A') = i o s2:1->S)’)
     |> rewr_rule[]) >> 
 ...
 >-- (strip_tac >> 
     accept_tac(forall_exists_dual |> qtinst_thm [(‘A’,‘1’),(‘B’,‘S’)] 
     |> qfVar_prpl_th1 ("P",[‘s1:1->S’],‘~(dom(f:2->A) = i o s1:1->S)’)
     |> rewr_rule[])) >>
 qby_tac
 ...
 >-- (strip_tac >>
     accept_tac(forall_exists_dual |> qtinst_thm [(‘A’,‘1’),(‘B’,‘S’)] 
     |> qfVar_prpl_th1 ("P",[‘s2:1->S’],‘~(cod(f:2->A) = i o s2:1->S)’)
     |> rewr_rule[])) >> 
 qsuff_tac
 ‘?(cf : fun(A, Cl)).
...
e0
(rpt strip_tac >> last_x_assum (strip_assume_tac o uex_expand) >>
 qsuff_tac ‘f1 = f & f2 = f’ >-- (strip_tac >> arw[]) >>
 strip_tac >>
 first_x_assum irule >> arw[])
(form_goal
 “(?!f:A->B. P[f:A->B](f)) ⇒
  ∀f1:A->B f2:A->B. P[f:A->B](f1) & P[f:A->B](f2) ⇒ f1 = f2”));
...
(rpt strip_tac >> rw[jointEpi2_def] >> rpt strip_tac >>
 ...
 drule
 (uex_unique |> qtinst_thm [(‘A’,‘P’),(‘B’,‘Y'’)]
             |> qfVar_prpl_th1 ("P",[‘a:P->Y'’],
...
local
val l = 
 CC5 |> qtinst_thm [(‘A’,‘C’),(‘B’,‘C’)]
 |> qfVar_prpl_th1 ("R",[‘fc1:2->C’,‘fc2:2->C’],
