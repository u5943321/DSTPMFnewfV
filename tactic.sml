...
fun drule0 optfn  th (G,fl:form list,f) = 
    let 
        ...
        fun mfn _ asm = 
            let 
                val menv = match_form (fvfl (ant th)) (flprops (ant th)) ant' asm mempty
...
fun match_mp_tac th (ct:cont,asl:form list,w) = 
    let
        ...
        val env = match_form (fvfl (ant th)) (HOLset.empty String.compare) con gl mempty
...
fun ind_with th (ct,asl,w) = 
    let 
        ...
        val (ante,con) = dest_imp conc
                         handle _ => (TRUE,conc)
        val (P,fvvl,args) = dest_fvar con
        val (qvs,b) = stripn_forall (length args) w
        val (gante,gcon) = dest_imp b
                           handle _ => (TRUE,b)
        val instf = mk_foralls qvs gcon
        val (_,vl) = strip_forall0 instf
        val sl = List.map #2 vl
        val th2 = fVar_prpl_th (mk_fd [("P",qvs,gcon)]) th
    in match_mp_tac th2 (ct,asl,w)
       handle _ => match_mp_tac (fVar_prpl_th (mk_fd [("P",qvs,b)]) th) (ct,asl,w)
    end
...
fun gen_full_simp_tac r tac thms:tactic =
    (pop_assum_list (f r tac thms):tactic) then_tac (arw_tac thms:tactic)
