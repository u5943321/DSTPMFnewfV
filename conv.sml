...
fun part_fmatch partfn th f = 
    let 
        val fvd = match_form (fvfl (ant th)) (flprops (ant th)) (partfn th) f mempty
    in 
        inst_thm fvd th
    end
...
fun fvar_fconv c f = 
    let val (P,vl,tl) = dest_fvar f
    in
        EQ_fvar P vl (mapUNCH [] c tl)
        handle _ => raise ERR ("fvar_fconv.not an fvar",[],[],[f])
    end
