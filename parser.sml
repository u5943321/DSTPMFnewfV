...
fun mk_pfVar P pvl ptl = pfVar(P,pvl,ptl)
...
handle _ => raise simple_fail "term_from_pt.no ground sort!"
...
fun rightsqparen (x, Key"]"::toks,env) = (x, toks,env)
  | rightsqparen _ = raise ERROR "Symbol ] expected";
...
datatype ast = 
         ...
         | afV of string * ast list * ast list
...
and parse_ast_atom tl = 
    case tl of
     ...
     | Id(a)::Key"["::tl1 => 
       let 
           val (astl,tl2) = rparen "(" (rparen "]" (parse_arepeat1 (",",parse_ast) tl1))
           val (argl,tl3) = rparen ")" (parse_arepeat1 (",",parse_ast) tl2)
       in (afV(a,astl,argl),tl3)
       end
...
fun ast2pt ast (env,n) = 
    case ast of 
      ...
      | afV _ => raise simple_fail "ast2pt.ast is a fVar, not a sort"
...
and ast2ps ast (env,n) = 
    case ast of
      ...
      | _ => raise PER ("ast2ps.wrong attempt of trying to turn an aBinder or afV into a pre-sort",[],[])
...
fun astl2ptl astl (env,n) = 
    case astl of 
        [] => ([],(env,n))
      | h :: t => 
        let val (pt,(env1,n1)) = ast2pt h (env,n)
            val (astl1,(env2,n2)) = astl2ptl t (env1,n1)
        in (pt :: astl1,(env2,n2))
        end 
...
fun ast2pf ast (env:env,n) = 
    case ast of 
      ...
      | afV(str,avl,aargl) =>
        let val (pvl,(env1,n1)) = astl2ptl avl (env,n)
            val (pargl,(env2,n2))  = astl2ptl aargl (env1,n1)
        in 
            (pfVar (str,pvl,pargl),(env2,n2))
        end 
      | aInfix(ast1,str,ast2) => 
        if mem str ["&","|","<=>","==>"] then
            let
...
f;
val it = "P[x:mem(A),y:mem(B)](a,b)": string
...
read_ast_pf 
 ...
 val it =
   (pfVar
     ("P",
      [pAnno (pVar ("x", psvar " 1"), psrt ("mem", [pVar ("A", psvar " 0")])),
       pAnno (pVar ("y", psvar " 3"), psrt ("mem", [pVar ("B", psvar " 2")]))],
      [pVar ("a", psvar " 0"), pVar ("b", psvar " 1")]),
    ((?, ?, ?, ?, 2), 0)): pform * (env * int)

wrong

(parse_ast_end (parse_ast (lex f))) 
...
fun form_from_pf env pf = 
    case pf of 
      ...
      | pPred("=",[t1,t2]) => 
        mk_eq (term_from_pt env t1)  (term_from_pt env t2)
      | pPred(P,ptl) => 
        mk_pred P (List.map (term_from_pt env) ptl)
      | pfVar(P,pvl,ptl) => mk_fvar P (List.map (dest_var o (term_from_pt env)) pvl) (List.map (term_from_pt env) ptl)
...
fun anno_ast (ns as (n,s)) ast = 
    case ast of
      ...
      | afV _ => raise simple_fail "anno_ast.ast is a fVar, not a term"
