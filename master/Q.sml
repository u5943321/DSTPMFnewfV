...
fun qfVar_prpl_th qpvsfl th = 
    let val pvsfl = List.map (fn(p,qvl,qf) => (p,List.map (dest_var o (qparse_term_with_cont essps)) qvl,
 qparse_form_with_cont essps qf)) qpvsfl
        val fd = mk_fd pvsfl
    in fVar_prpl_th fd th
    end
...
fun qmk_tinst ql = 
    let val sstl = 
            List.map 
                (fn (qv,qt) =>
                    (dest_var (qparse_term_with_cont essps qv),qparse_term_with_cont essps qt)) ql
    in mk_inst sstl []
    end 
