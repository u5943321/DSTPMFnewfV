...
fun fvsl sl = var_bigunion (List.map fvs sl)
...
fun tprpl i (bmap:term list) t = 
    case t of 
        Var _ => t
      | Fun (f,s,tl) => 
        Fun (f,sprpl i bmap s,
             List.map (tprpl i bmap) tl)
      | Bound j => 
        if 0 <= j - i andalso j - i < length bmap then 
            List.nth(bmap,j - i) else t
and sprpl i bmap (srt (sname,tl)) = 
    srt (sname,List.map (tprpl i bmap) tl)
...
fun tshift i tm = 
    case tm of 
        Var _ => tm
      | Fun(f,s,tl) => Fun(f,sshift i s,List.map (tshift i) tl) 
      | Bound j => Bound (j + i) 
and sshift i (srt (sname,tl)) = 
    srt (sname,List.map (tshift i) tl)



fun tabs (v as (m,s)) i (Var(n,st)) =
    if m = n andalso s = st 
    then Bound i else Var(n,st)
  | tabs (m,s) i (Fun(f,st,tl)) =
    Fun(f,sabs (m,s) i st,List.map (tabs (m,s) i) tl)
  | tabs (m,s) t (Bound i) = Bound i 
and sabs (m,s) i (srt(n,tl)) =
    srt(n,List.map (tabs (m,s) i) tl)

fun delete_fsym f = 
let val (d1,_) = Binarymap.remove (!fsyms,f) 
in fsyms := d1
end
end
